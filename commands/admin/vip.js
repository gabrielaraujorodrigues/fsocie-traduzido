...
export default {
  ...
  command: ["vip"],
  category: "admin",
  description: "Administra usuários VIP con tempo, usos y painel mejorado",
  ownerOnly: true,
  ...
  run: async ({ sock, msg, from, args = [], settings }) => {
    ...
    if (action === "del" || action === "remove" || action === "rm") {
      ...
      delete data.users[number];
      saveVip(data);
      return sock.sendMessage(from, { text: `VIP removido: *${number || "sin número"}*`, ...global.channelInfo }, { quoted: msg });
    }

    return sock.sendMessage(
      from,
      {
        text: `Subcomando invalido. Use ${prefix}vip`,
        ...global.channelInfo,
      },
      { quoted: msg }
    );
