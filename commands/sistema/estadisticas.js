...
export default {
  name: "estatísticas",
  command: ["estatísticas", "stats", "botstatsfull"],
  category: "sistema",
  description: "Mostra estatísticas de uso del bot",
  ...
  run: async ({ sock, msg, from, args = [], esDono }) => {
    if (!esDono) {
      return sock.sendMessage(
        from,
        {
          text: "Solo el owner puede ver las estatísticas del bot.",
          ...global.channelInfo,
        },
        { quoted: msg }
      );
    ...
    if (!runtime?.getUsageStats) {
      return sock.sendMessage(
        from,
        {
          text: "No pude acceder a las estatísticas internas.",
          ...global.channelInfo,
        },
        { quoted: msg }
      );
