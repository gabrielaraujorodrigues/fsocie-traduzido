    ...
    if (action === "catalogo" || action === "wacatalogo" || action === "wa_catalogo") {
      ...
      if (!targetPhone) {
        return sendFallback(
          ...
          from,
          msg,
          `No encontre número business para abrir catalogo.\n\nUso:\n${prefix}catalogo 519XXXXXXXX`
        );
      }
      ...
      return sock.sendMessage(
        from,
        {
          text:
            "*Catalogo WhatsApp*\n" +
            `Negocio: +${targetPhone}\n\n` +
            "Pressione el boton para abrir el catalogo del negocio.",
          title: "Catalogo WA",
    ...
    if (action === "producto") {
      ...
      if (!productId) {
        return sendFallback(
          ...
          from,
          msg,
          `Falta product_id.\n\nUso:\n${prefix}producto <product_id> [número_business]`
        );
      }
    ...
    if (action === "catalogolista" || action === "catalogolist") {
      ...
      if (!bizJid || productIds.length === 0) {
        return sendFallback(
          ...
          from,
          msg,
          `Uso:\n${prefix}catalogolista <número_business|jid> <product_id1,product_id2,...>`
        );
      }
      ...
      return sock.sendMessage(
        from,
        {
          ...
          text: "Selecione productos del catalogo del negocio.",
