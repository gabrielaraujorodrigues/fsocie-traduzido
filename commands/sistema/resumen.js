...
function getQuotedMedia(message = {}) {
  ...
  const áudio = message?.áudioMessage || message?.message?.áudioMessage;
  if (áudio) return { type: "áudio", media: áudio };

  const vídeo = message?.vídeoMessage || message?.message?.vídeoMessage;
  if (vídeo) return { type: "vídeo", media: vídeo };
...
export default {
  name: "resumo",
  command: ["resumo", "sumario", "brief"],
  category: "sistema",
  description: "Resumo rapidamente sin IA: texto o metadatos de áudio/vídeo",
  ...
  run: async (ctx) => {
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `MIME: *${mime}*\n\n` +
            `Nota: para resumo por contenido de áudio se necesita transcripcion (API/STT).`,
          ...global.channelInfo,
        },
        quoted
      );
    ...
    if (!targetText) {
      return sock.sendMessage(
        from,
        {
          text:
            `Uso:\n` +
            `• ${prefix}resumo <texto largo>\n` +
            `• O responda un texto y usa: ${prefix}resumo\n` +
            `• O responda un áudio/vídeo para resumo de metadatos`,
          ...global.channelInfo,
        },
        quoted
      );
    ...
    if (!data) {
      return sock.sendMessage(
        from,
        {
          text: "No pude generar resumo de ese texto.",
          ...global.channelInfo,
        },
        quoted
      );
