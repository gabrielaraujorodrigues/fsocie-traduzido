...
export default {
  ...
  description: "Perfil completo: níveles, economia, premium y actividad.",
  ...
  run: async ({ sock, msg, from, sender, settings = {}, isGroup = false }) => {
    ...
    const levelBlock = levelProfile
      ? `🏅 *NIVEL GRUPO*\n` +
        `• Nível: *${levelProfile.level}* | Rol: *${levelProfile.role}*\n` +
        ...
        `• Mensagems/Comandos: *${levelProfile.messages}/${levelProfile.commands}*`
      : `🏅 *NIVEL GRUPO*\n• Disponível al usar este comando dentro de un grupo`;

    const text =
      `╭━━━〔 👤 PERFIL FSOCIETY 〕━━━⬣\n` +
      `┃ Usuário: *${formatUser(sender)}*\n` +
      `┃ Nome: *${cleanText(economy?.lastKnownName || "") || "Sin nome"}*\n` +
      `╰━━━━━━━━━━━━━━━━━━━━━━⬣\n\n` +
      `${levelBlock}\n\n` +
      `💰 *ECONOMIA*\n` +
      ...
      `💎 *PREMIUM*\n` +
      `• Nível premium: *${premium.level}* | XP nível: *${premium.xpInLevel}*\n` +
      ...
      `• Próximo hito actividad: *${premium.nextActivityMilestone || "N/D"}*\n\n` +
      `Tip: use *${prefix}premium* y *${prefix}níveles* para mas detalhe.`;
