...
function buildErroMessage(erro, contactText = "") {
  const message = String(erro?.message || erro || "Erro desconocido");
...
function resolveMode(args = []) {
  ...
  if (["lite", "rapidamente", "rápido", "fast"].includes(mode)) {
    return {
      modeLabel: "RÁPIDO",
...
export default {
  ...
  run: async ({ sock, msg, from, args = [], settings = {} }) => {
    ...
    const ownerName = buildDonoContact(settings);
    ...
    try {
      ...
      await sock.sendMessage(
        from,
        {
          text: buildErroMessage(erro, ownerName),
          ...global.channelInfo,
        },
        { quoted: msg }
      );
