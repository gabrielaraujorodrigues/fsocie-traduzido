...
function buildChannelSummary(meta, inviteCode) {
  ...
  const lines = [
    ...
    owner ? `Dono JID: *${owner}*` : "",
    ownerLid ? `Dono LID: *${ownerLid}*` : "",
...
export default {
  ...
  description: "Obtiene JID/LID de canal por link o convierte número a JID/LID",
  ...
  run: async ({ sock, msg, from, args = [], settings, sender = "" }) => {
    ...
    return sock.sendMessage(
      from,
      {
        text: "Entrada invalida. Envia un link de canal, JID @newsletter o número.",
        ...global.channelInfo,
      },
      { quoted: msg }
    );
