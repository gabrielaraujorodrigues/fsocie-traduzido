...
async function buildFigurinhaFromGifUrl(gifUrl) {
  await ensureTmpDir();
...
export default {
  ...
  command: ["gif"],
  category: "media",
  description: "Busca un GIF en Tenor y lo envia en figurinha",
  ...
  run: async ({ sock, msg, from, args = [], settings }) => {
    ...
    try {
      ...
      for (const item of selectedResults) {
        ...
        if (!gifUrl) continue;
        const figurinhaBuffer = await buildFigurinhaFromGifUrl(gifUrl);
        await sock.sendMessage(
          from,
          { figurinha: figurinhaBuffer, ...global.channelInfo },
          quoted
        );
      }
      ...
      const message = cleanText(
        erro?.message || "No pude generar el figurinha GIF en este momento."
      );
      return sock.sendMessage(
        from,
