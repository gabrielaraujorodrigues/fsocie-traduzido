      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `Archivos excluídos: *${result.removedFiles}*\n` +
            `Carpetas vacias borradas: *${result.removedDirs || 0}*\n` +
            `Espacio liberado: *${result.freedLabel}*\n` +
            `TMP downloads limpiado: *${result.managedTempRemovedFiles || 0} archivos / ${result.managedTempFreedLabel || formatBytes(0)}*`,
          ...global.channelInfo,
        },
        { quoted: msg }
      );
    ...
    return sock.sendMessage(
      from,
      {
        text:
          ...
          `Edad maxima: *${formatDuration(state.maxFileAgeMs)}*\n` +
          `Limite TMP: *${formatTmpLimit(state.maxTmpTotalBytes)}*\n` +
          `Ultima execução: *${state.lastRunAt ? new Date(state.lastRunAt).toLocaleString("es-PE") : "Nunca"}*\n` +
          `Último excluído: *${state.lastSummary.removedFiles} archivos / ${formatBytes(state.lastSummary.freedBytes)}*\n` +
          `TMP downloads: *${state.lastSummary.managedTempRemovedFiles || 0} archivos / ${state.lastSummary.managedTempFreedLabel || formatBytes(0)}*\n` +
