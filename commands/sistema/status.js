...
function buildMainPainel({ settings, comandos, vipCount }) {
...
function buildGroupPainel({ welcomeOn, modoAdmiOn, antilinkOn, antifakeOn }) {
  return [
    "╭─〔 CONTROL GRUPO 〕",
...
export default {
  ...
  category: "sistema",
  description: "Painel de estado del bot",
  ...
  run: async ({ sock, msg, from, settings, comandos, esGrupo }) => {
    ...
    const modoAdmiOn = readFileState(path.join(dbDir, "modoadmin.json"), from, false);
    ...
    const sections = [
      buildMainPainel({ settings, comandos, vipCount }),
      esGrupo
        ? buildGroupPainel({ welcomeOn, modoAdmiOn, antilinkOn, antifakeOn })
        : [
            "╭─〔 CHAT PRIVADO 〕",
            "│ Painel abierto en mensagem privado.",
            "╰──────────────⬣",
          ].join("\n"),
    ];
