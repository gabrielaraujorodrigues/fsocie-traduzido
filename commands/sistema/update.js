...
function resolveDonoAccess({ esDono, settings, msg, from }) {
  const ownerIds = collectDonoIds(settings);
...
function pickMainLine(result) {
  ...
  return lines[0] || "Sin detalhe extra.";
}
...
async function tryApplyHotRuntimeUpdate() {
  ...
  try {
    ...
    return {
      ...
      message: detail?.ok
        ? "Comandos y settings recarregados en caliente."
  ...
  return new Promise((resolve, reject) => {
    const request = https.get(
      ...
      (response) => {
        ...
        if (statusCode !== 200) {
          response.resume();
          reject(new Erro(`GitHub respondio con estado ${statusCode}.`));
          return;
        }
...
function getExtractedSourceRoot(directory) {
  ...
  if (!entries.length) {
    throw new Erro("La download del repo no trajo archivos para aplicar.");
  }
    ...
    setTimeout(async () => {
      ...
      if (!result.ok) {
        console.erro(
          ...
          String(result?.stderr || result?.stdout || "sin detalhe").trim()
        );
        process.exit(0);
        return;
      }
  ...
  if (gitRepoAvailable) {
    try {
      ...
      return {
        ...fallbackResult,
        fallbackNote:
          `git pull falha y use GitHub directo como respaldo.\n` +
          `Motivo: ${String(gitErro?.message || gitErro || "sin detalhe").slice(0, 220)}`,
      };
    }
  }
...
async function performArchiveUpdate({
  ...
  try {
    ...
    return {
      ...
      depsInstalled,
      detailLine: `Snapshot descarregado desde ${resolvedSource.repoId}`,
...
async function buildUpdateInfo(settings, msg, from, esDono) {
  const ownerAccess = resolveDonoAccess({ esDono, settings, msg, from });
  const gitRepoAvailable = await isInsideGitWorkTree();
...
export default {
  ...
  category: "sistema",
  description:
    "Actualiza el bot con git pull cuando el repo local esta disponível; si falla use GitHub directo como respaldo",
        ...
        return sock.sendMessage(
          from,
          {
            text: buildUpdateMessage(
              "UPDATE INFO",
              [
                `• Dono detectado: *${info.ownerAccess.isDono ? "SI" : "NO"}*`,
                `• Matches owner: *${info.ownerAccess.matches.join(", ") || "nenhum"}*`,
                `• Sender IDs: ${info.ownerAccess.senderIds.join(", ") || "nenhum"}`,
                `• Donos config: ${info.ownerAccess.ownerIds.join(", ") || "nenhum"}`,
        ...
        return sock.sendMessage(
          from,
          {
            text: buildUpdateMessage("ERROR UPDATE INFO", [
              `${erro?.message || "No pude revisar el estado del bot."}`,
            ]),
            ...global.channelInfo,
          },
          quoted
        ...
        from,
        {
          text: buildUpdateMessage(
            ...
            [
              ...
              `• Sender detectado: *${ownerAccess.senderIds.join(", ") || "nenhum"}*`,
              `• Donos salvos: *${ownerAccess.ownerIds.join(", ") || "nenhum"}*`,
            ],
      ...
      await sock.sendMessage(
        from,
        {
          text: buildUpdateMessage(
            ...
            [
              skipRestart
                ? `• ${gitRepoAvailable ? "Actualizando con git pull" : "Downloadndo desde GitHub"} sin reiniciar el proceso`
              ...
              skipRestart
                ...
                : "• Reinicio automático: *SI, solicitado por owner*",
            ...
            text: buildUpdateMessage(
              ...
              [
                updateResult.mode === "git"
                  ...
                  : "No detecte archivos novos para copiar desde GitHub.",
                updateResult.mode === "git"
          ...
          from,
          {
            text: buildUpdateMessage(
              ...
              [
                ...
                `• Detalhe: ${updateResult.detailLine}`,
                ...
                ...(hotReloadResult.attempted ? [`• Detalhe hot: ${hotReloadResult.message}`] : []),
                "• Aplicacion: *sin reinicio*",
              ],
              restartNeeded
                ? "Los archivos ya se actualizaron en disco. Aun hay cambios que requieren `.restart` para quedar ativos."
                : "Los cambios quedaron aplicados y carregados sin reinicio."
            ),
            ...global.channelInfo,
          },
          quoted
        ...
        await sock.sendMessage(
          from,
          {
            text: buildUpdateMessage(
              ...
              "Este hosting no use reinicio interno seguro desde el bot. Los archivos ya quedaron atualizados, pero debes reiniciar desde tu painel, PM2 o consola cuando quieras cargar el codigo novo."
            ),
            ...global.channelInfo,
          },
          quoted
        ...
        await sock.sendMessage(
          from,
          {
            text: buildUpdateMessage(
              ...
              "Bloquee el reinicio interno para no tumbar tu servidor. Reinicia manualmente cuando quieras cargar el codigo novo."
            ),
            ...global.channelInfo,
          },
          quoted
      ...
      await sock.sendMessage(
        from,
        {
          text: buildUpdateMessage("ERROR UPDATE", [
            `${erro?.message || "No pude actualizar el bot."}`,
          ]),
          ...global.channelInfo,
        },
        quoted
