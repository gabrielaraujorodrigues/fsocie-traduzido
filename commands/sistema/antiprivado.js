...
function normalizeAction(raw = "") {
  ...
  if (["on", "encender", "ativar", "enable", "1", "si"].includes(value)) return "on";
  if (["off", "desligar", "desativar", "disable", "0", "no"].includes(value)) return "off";
...
export default {
    ...
    if (action === "status") {
      return sock.sendMessage(
        from,
        {
          text:
            `🔐 *ANTIPRIVADO*\n\n` +
            `Estado atual: *${state.enabled ? "ON ✅" : "OFF ❌"}*`,
          ...global.channelInfo,
        },
        quoted
      );
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `El bot vuelve a respondar privados para todos.`,
          ...global.channelInfo,
        },
        quoted
      );
  ...
  onMessage: async ({ msg, esDono, isGroup, sender, senderPhone, senderLid, settings, sock }) => {
    ...
    // Bloquea en silencio privados de usuários normales.
    return true;
  },
};
