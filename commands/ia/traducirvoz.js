...
function getQuotedÁudioMessage(quoted = null) {
  if (!quoted || typeof quoted !== "object") return null;
  const áudio =
    quoted?.message?.áudioMessage ||
    quoted?.áudioMessage ||
    quoted?.message?.pttMessage ||
    null;
  if (!áudio) return null;
...
async function translateText(text, source, target) {
  try {
    ...
    try {
      ...
      throw new Erro(
        `No pude traducir ahora mismo. Google: ${googleErro?.message || googleErro} | MyMemory: ${memoryErro?.message || memoryErro}`
      );
    }
  }
}
...
async function transcribeWithEndpoint(sttUrl, áudioBuffer, mimeType = "áudio/ogg", sourceLang = "auto") {
  ...
  try {
    ...
    const blob = new Blob([áudioBuffer], { type: mimeType || "áudio/ogg" });
    ...
    if (!text) throw new Erro("Respuesta sin texto de transcripcion");
    return text;
  } catch (erro) {
  ...
  try {
    const payload = {
      áudio_base64: Buffer.from(áudioBuffer).toString("base64"),
      mime: mimeType || "áudio/ogg",
      language: sourceLang,
    };
    const response = await fetch(endpoint, {
      method: "POST",
  ...
  throw new Erro(`No pude transcribir áudio con ese endpoint. ${erros.join(" | ")}`);
}
  ...
  run: async (ctx) => {
    ...
    if (!supported[parsed.target]) {
      return sock.sendMessage(
        from,
        {
          text: `Idioma destino no valido. Disponíveis: ${Object.keys(supported).join(", ")}`,
          ...global.channelInfo,
        },
        quoted
      );
    ...
    if (!sourceText) {
      const quotedÁudio = getQuotedÁudioMessage(quotedMessage);
      if (!quotedÁudio) {
        return sock.sendMessage(
          from,
          {
            text:
              `Uso:\n` +
              `• Responda a una nota de voz y usa: ${prefix}traducirvoz en\n` +
      ...
      try {
        const áudioBuffer = await downloadMediaMessage(
          quotedÁudio,
          "buffer",
          {},
        ...
        const mimeType =
          quotedÁudio?.message?.áudioMessage?.mimetype ||
          quotedÁudio?.message?.pttMessage?.mimetype ||
          "áudio/ogg";
