...
function resolveMenuImagePath() {
  const base = path.join(process.cwd(), "imagens", "menu");
...
function resolveCategoryImagePath(category = "") {
  ...
  const baseByCategory = {
    ...
    ferramentas: path.join(imageDir, "menu-sistema"),
...
function getCategoryDescription(category = "", count = 0) {
  ...
  const descriptions = {
    menu: "Painel principal del bot",
    downloads: "Áudio, vídeo y downloads",
    free_streaming_accounts: "Contas de streaming grátis e acesso compartilhado",
    busca: "Busca y resultados rapidamentes",
    freefire: "Utilitários para Free Fire",
    jogos: "Diversão y minijogos",
    ferramentas: "Ferramentas y utilitários",
    grupos: "Ajustes e controle de grupos",
    subbots: "Gestão de subbots",
    economia: "Sistema econômico do bot",
    sistema: "Estado, atualização e controle",
    ia: "Funciones de intescolhancia artificial",
    media: "Imagem, figurinhas y multimídia",
    anime: "Comandos de anime",
    admin: "Comandos administrativos",
    owner: "Funções exclusivas do dono",
    vip: "Funções premium ou vip",
    otros: "Otros comandos disponíveis",
  };
...
function buildCategoryBlock(category, commands, primaryPrefix) {
  ...
  const lines = [
    ...
    `│ Mix de acesso: PÚBLICO ${highlight.accessCounts.PÚBLICO} • ADMIN ${highlight.accessCounts.ADMIN} • OWNER ${highlight.accessCounts.OWNER}`,
...
function makeSingleCaption(fullCaption, primaryPrefix) {
  ...
  return (
    ...
    "╭─〔 ⚠️ *MENU CORTADO* 〕\n" +
    "┃ Hay demaiss comandos para un solo mensagem.\n" +
    `┃ Use ${primaryPrefix}menu para ver o principal.\n` +
    "╰────────────⬣"
  );
}
...
function buildCategorySections(categoryNames, categories, primaryPrefix) {
  ...
  if (!sections.length) {
    return [{ title: "Categorias do bot", rows: buildCategoryRows(categoryNames, categories, primaryPrefix) }];
  }
  ...
  const flowButton = {
    ...
    nativeFlowInfo: {
      ...
      paramsJson: JSON.stringify({
        title: "☠️ FSOCIETY-V1 SELETOR DE COMANDOS",
        sections,
      }),
    },
  };
  ...
  const quickButtons = [
    {
      ...
      buttonText: { displayText: "↩ EQUIPE BOT" },
      type: 1,
    },
    {
      buttonId: `${primaryPrefix}gruposoficiales`,
      buttonText: { displayText: "↩ COMUNIDADE" },
      type: 1,
    },
  ];
...
function buildMenuLandingText(menuContext, settings, uptime, totalCategories, totalCommands, prefixLabel) {
  ...
  return [
    ...
    "║ Pressione *ABRIR MENU* para expandir categorias.",
    "╠════════════════════════════════════════════╣",
    `║ 👤 Visão: *${menuContext.subtitle}*`,
...
export default {
  ...
  categoria: "menu",
  description: "Mostra el menú principal del bot.",
  ...
  run: async ({ sock, msg, from, settings, comandos, botId, botLabel, args = [] }) => {
    try {
      ...
      const topPainel = buildTopPainel({
        settings,
        uptime,
      ...
      await sock.sendMessage(
        from,
        {
          text:
            ...
            `┃ ${String(erro?.message || "Erro desconocido")}\n` +
            "╰━━━━━━━━━━━━━━━━━━━━⬣",
          ...global.channelInfo,
        },
        { quoted: msg }
