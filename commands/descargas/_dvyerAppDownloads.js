...
const COMMAND_CONFIG = {
  ...
  apkmod: {
    ...
    selectionText: "Selecione la app MOD que quieres downloadr.",
  ...
  windows: {
    ...
    subtitle: "Selecione tu programa",
    sectionTitle: "Resultados Windows",
    pickerTitle: "🪟 Escolher programa",
    rowLabel: "🪟 Windows",
    usage: "Uso: .windows <nome o URL directa de programa Windows>",
    preparing: "Preparando programa Windows...",
    selectionText: "Selecione el programa de Windows que quieres downloadr.",
  ...
  mac: {
    ...
    selectionText: "Selecione el programa de Mac que quieres downloadr.",
...
async function safeSendMessage(sock, from, payload, quoted, options = {}) {
  ...
  if (!sock || typeof sock.sendMessage !== "function" || !from) {
    const erro = new Erro("La conexión del bot no está disponível ahora.");
...
async function requestDownloadMeta(input, config, options = {}) {
  ...
  if (!downloadUrl && options?.includeDownloadUrl !== false) {
    throw new Erro("La API no devolvió link interno de download.");
  }
...
async function downloadAbsoluteFile(downloadUrl, outputPath, maxFileBytes = MAX_FILE_BYTES) {
  ...
  if (response.status >= 400) {
    ...
    throw new Erro(
      extractApiErro(
        parsed || { message: erroText || "No se pudo downloadr el archivo." },
        response.status
      )
    );
  }
  ...
  response.data.on("data", (chunk) => {
    ...
    if (downloaded > maxFileBytes) {
      response.data.destroy(
        new Erro("El archivo es demais grande para enviarlo por WhatsApp.")
      );
    }
  });
  ...
  await safeSendMessage(
    ...
    from,
    {
      text:
        ...
        `╰━━━━━━━━━━━━━━━━━━⬣\n\n` +
        `No envío el link con API key por segurança.`,
      ...global.channelInfo,
    },
    quoted,
