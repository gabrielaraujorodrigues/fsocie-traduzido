...
function safeFileName(name) {
  return (
    String(name || "facebook-vídeo")
      .replace(/[\\/:*?"<>|]/g, "")
      .replace(/\s+/g, " ")
      .trim()
      .slice(0, 100) || "facebook-vídeo"
  );
}

function normalizeMp4Name(name) {
  const clean = safeFileName(String(name || "facebook-vídeo").replace(/\.mp4$/i, ""));
  return `${clean || "facebook-vídeo"}.mp4`;
}
...
async function requestFacebookMeta(vídeoUrl) {
  ...
  return {
    title: safeFileName(data?.title || data?.result?.title || "Facebook Vídeo"),
    ...
    fileName: normalizeMp4Name(
      data?.filename || data?.file_name || data?.result?.filename || "facebook-vídeo.mp4"
    ),
    downloadUrl: pickApiDownloadUrl(data),
  };
}

async function downloadFacebookVídeo(vídeoUrl, outputPath, directUrl = "") {
  ensureTmpDir();
  ...
  if (response.status >= 400) {
    ...
    throw new Erro(
      extractApiErro(
        parsed || { message: erroText || "No se pudo downloadr el vídeo." },
        response.status
      )
    );
  }
...
async function sendVídeoOrDocument(sock, from, quoted, options) {
  const {
    filePath,
    fileName,
    title,
  ...
  try {
    ...
    console.erro("send vídeo failed:", erro?.message || erro);
...
export default {
  ...
  category: "download",
  description: "Download vídeos públicos de Facebook usando DVYER API.",
  ...
  run: async (ctx) => {
    ...
    try {
      ...
      const vídeoUrl = extractFacebookUrl(rawInput);
        ...
        return sock.sendMessage(
          from,
          {
            text:
              ...
              "┃ También puedes respondar a un mensagem con el link.\n" +
              "╰━━━━━━━━━━━━━━━━━━⬣",
            ...global.channelInfo,
          },
          quoted
      ...
      const info = await requestFacebookMeta(vídeoUrl);
      ...
      const downloaded = await downloadFacebookVídeo(
        vídeoUrl,
        tempPath,
        info.downloadUrl
      );
      ...
      await sendVídeoOrDocument(sock, from, quoted, {
      ...
      await sock.sendMessage(
        from,
        {
          text:
            "╭━━〔 ❌ *ERROR FACEBOOK* 〕━━⬣\n" +
            `┃ ${sanitizeProviderMessage(erro, { kind: "vídeo", fallback: "No se pudo procesar el vídeo de Facebook." })}\n` +
            "╰━━━━━━━━━━━━━━━━━━⬣",
          ...global.channelInfo,
        },
        quoted
