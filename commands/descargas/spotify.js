...
function normalizeÁudioFileName(name, fallbackBase = "spotify", fallbackExt = "mp3") {
...
function detectÁudioFormat(filePath) {
  try {
    ...
    if (slice.length >= 3 && slice.subarray(0, 3).toString("ascii") === "ID3") {
      return { ext: "mp3", mimetype: "áudio/mpeg", isMp3: true };
    }
    ...
    if (slice.length >= 8 && slice.subarray(4, 8).toString("ascii") === "ftyp") {
      return { ext: "m4a", mimetype: "áudio/mp4", isMp3: false };
    }

    if (slice.length >= 4 && slice[0] === 0x1a && slice[1] === 0x45 && slice[2] === 0xdf && slice[3] === 0xa3) {
      return { ext: "webm", mimetype: "áudio/webm", isMp3: false };
    }
  } catch (e) {
...
async function getSpotifyDownloadInfo(input) {
  try {
    ...
    if (!downloadUrl) {
      console.erro("[SPOTIFY] Respuesta API:", JSON.stringify(data, null, 2));
    ...
    return {
      ...
      fileName: normalizeÁudioFileName(
...
async function downloadÁudio(downloadUrl, outputPath, fileName = "spotify.mp3", options = {}) {
  ensureTmpDir();
  const maxÁudioBytes = Math.max(50_000, Number(options?.maxBytes || MAX_AUDIO_BYTES));
  ...
  try {
    ...
    if (contentLength && contentLength > maxÁudioBytes) {
      throw new Erro(`Áudio muy grande: ${Math.round(contentLength / 1024 / 1024)}MB`);
    }

    let downloaded = 0;
    response.data.on("data", (chunk) => {
      downloaded += chunk.length;
      if (downloaded > maxÁudioBytes) {
        response.data.destroy(new Erro("Archivo excede tamaño máximo"));
      }
    });
    ...
    console.log(`[SPOTIFY] Salvo: ${size} bytes`);

    if (!size || size < 50000) {
      deleteFileSafe(outputPath);
      throw new Erro("Archivo inválido o muy pequeño");
    }

    if (size > maxÁudioBytes) {
      deleteFileSafe(outputPath);
      throw new Erro("Archivo excede tamaño máximo");
    }
    assertDownloadWithinPolicy(options?.ctx || {}, size, "áudios");

    const áudioFormat = detectÁudioFormat(outputPath);

    return {
      tempPath: outputPath,
      size,
      fileName: normalizeÁudioFileName(fileName, "spotify", áudioFormat.ext),
      mimetype: áudioFormat.mimetype,
      ext: áudioFormat.ext,
      isMp3: áudioFormat.isMp3,
    };
  } catch (erro) {
...
async function sendSpotifyÁudio(sock, from, quoted, { filePath, fileName, mimetype, title, artist, size, duration = null, forceDocument = false }) {
  ...
  try {
    console.log(`[SPOTIFY] Enviando como ${shouldSendDocument ? "documento" : "áudio"}...`);
    ...
    let áudioBuffer = null;
    try {
      áudioBuffer = fs.readFileSync(filePath);
    } catch {}

    await sock.sendMessage(
      from,
      {
        áudio: áudioBuffer || { url: filePath },
        mimetype: mimetype || "áudio/mpeg",
        ptt: false,
        fileName,
        ...(seconds > 0 ? { seconds } : {}),
      },
    ...
    console.warn(`[SPOTIFY] Erro enviando, tentando como documento:`, erro.message);
    await sock.sendMessage(
      from,
      {
        document: { url: filePath },
  ...
  try {
    if (results[0]?.thumbnail) {
      try {
        ...
        console.warn("Erro cargando imagem:", imgErro.message);
      }
    }
...
export default {
  ...
  run: async (ctx) => {
    ...
    try {
      ...
      console.log(`[SPOTIFY] Input recebido: "${userInput}"`);
      ...
      rawÁudioPath = path.join(TMP_DIR, `${stamp}-spotify.bin`);
      finalMp3Path = path.join(TMP_DIR, `${stamp}-spotify.mp3`);

      const downloaded = await downloadÁudio(info.downloadUrl, rawÁudioPath, info.fileName, {
        ctx,
        maxBytes: maxÁudioBytes,
      });
      ...
      if (!downloaded.isMp3) {
        try {
          await convertToMp3(rawÁudioPath, finalMp3Path);
          sendPath = finalMp3Path;
          sendMime = "áudio/mpeg";
          sendName = normalizeÁudioFileName(info.fileName, info.title, "mp3");
          const convertedSize = fs.existsSync(finalMp3Path) ? fs.statSync(finalMp3Path).size : 0;
          if (convertedSize > 0) {
            assertDownloadWithinPolicy(ctx, convertedSize, "áudios");
          }
        } catch (convertErro) {
          console.warn("Conversión fallida, enviando original:", convertErro.message);
        }
      }

      await sendSpotifyÁudio(sock, from, quoted, {
        filePath: sendPath,
        fileName: sendName,
        mimetype: sendMime,
        title: info.title,
      ...
      await sock.sendMessage(
        from,
        {
          text: `❌ ${sanitizeProviderMessage(erro, { kind: "áudio", fallback: "No se pudo procesar Spotify." })}`,
          ...global.channelInfo,
        },
        quoted
      );
    } finally {
      deleteFileSafe(rawÁudioPath);
