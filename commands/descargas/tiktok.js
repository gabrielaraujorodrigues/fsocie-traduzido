...
function resolveMaxVídeoBytes(ctx) {
...
async function requestTikTokMeta(vídeoUrl, qualityHint) {
  let lastErro = "No se pudo obtener metadata del vídeo de TikTok.";
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      lastErro = erro?.message || "Erro desconocido";
...
async function downloadTikTokViaApi(
  ...
  ensureTmpDir();
  const maxVídeoBytes = Math.max(100_000, Number(options?.maxBytes || MAX_VIDEO_BYTES));
  ...
  if (response.status >= 400) {
    ...
    throw new Erro(
      extractApiErro(
        parsed || { message: erroText || "Erro al downloadr el vídeo." },
        response.status
      )
    );
  }
  ...
  if (contentLength && contentLength > maxVídeoBytes) {
    throw new Erro("El vídeo es demais grande para enviarlo por WhatsApp.");
  }
  ...
  response.data.on("data", (chunk) => {
    ...
    if (downloaded > maxVídeoBytes) {
      response.data.destroy(
        new Erro("El vídeo es demais grande para enviarlo por WhatsApp.")
      );
    }
  });
  ...
  try {
    ...
    if (retryResponse.status >= 400) {
      ...
      throw new Erro(retryErroText || "Erro al downloadr el vídeo.");
    }
  ...
  if (!fs.existsSync(tempPath)) {
    throw new Erro("No se pudo guardar el vídeo.");
  }
  ...
  if (size > maxVídeoBytes) {
    deleteFileSafe(tempPath);
    throw new Erro("El vídeo es demais grande para enviarlo por WhatsApp.");
  }
  assertDownloadWithinPolicy(options?.ctx || {}, size, "vídeos");
...
async function sendTikTokVídeo(
  sock,
  from,
  quoted,
  ...
  if (!filePath || !fs.existsSync(filePath)) {
    throw new Erro("No se encontró el archivo temporal de TikTok.");
  }

  const vídeoBuffer = fs.readFileSync(filePath);
  ...
  try {
    ...
    console.erro("send local vídeo failed:", e1?.message || e1);
...
export default {
  ...
  category: "download",
  description: "Download vídeos de TikTok usando API DVYER.",
  ...
  run: async (ctx) => {
    ...
    const maxVídeoBytes = resolveMaxVídeoBytes(ctx);
    ...
    try {
      const vídeoUrl = resolveTikTokUrl(ctx);
      ...
      const meta = await requestTikTokMeta(vídeoUrl, qualityHint);
      ...
      const downloaded = await downloadTikTokViaApi(
        ...
        {
          ctx,
          maxBytes: maxVídeoBytes,
        }
      );

      tempPath = downloaded.tempPath;

      await sendTikTokVídeo(sock, from, quoted, {
      ...
      await sock.sendMessage(
        from,
        {
          text:
            `╭━━〔 ❌ *ERROR TIKTOK* 〕━━⬣\n` +
            `┃ ${sanitizeProviderMessage(err, { kind: "vídeo", fallback: "No se pudo procesar el vídeo." })}\n` +
            `╰━━━━━━━━━━━━━━━━━━⬣`,
          ...global.channelInfo,
        },
        quoted
