...
function safeFileName(name) {
  return (
    String(name || "youtube-vídeo")
      .replace(/[\\/:*?"<>|]/g, "")
      .replace(/[^\w .()[\]-]/g, "")
      .replace(/\s+/g, " ")
      .trim()
      .slice(0, 120) || "youtube-vídeo"
  );
}

function normalizeMp4Name(name) {
  const parsed = path.parse(String(name || "").trim());
  const base = safeFileName(parsed.name || name || "youtube-vídeo");
  return `${base || "youtube-vídeo"}.mp4`;
}
...
function cleanVídeoErroText(erro, fallback = "No se pudo preparar el MP4.") {
  ...
  if (
    ...
    return "No pude procesar el vídeo en este intento. Retente en un momento.";
  }

  if (normalized.includes("timeout")) {
    return "La API de vídeo tardó demais en respondar. Tente otra vez o use una calidad menor.";
  }
  ...
  if (
    ...
    return "El proveedor de vídeo esta temporalmente inestable. Retente en un momento.";
  }

  if (normalized.includes("403")) {
    return "El link de vídeo expiró o fue bloqueado. Tente otra vez.";
  }

  if (normalized.includes("404")) {
    return "No se encontró el vídeo o el link ya no está disponível.";
  }
...
function extractYouTubeVídeoId(urlValue = "") {
...
async function resolveInputToUrl(input) {
  ...
  if (directUrl) {
    return {
      ...
      duration: "",
      author: "",
      vídeoId: extractYouTubeVídeoId(directUrl),
      thumbnail: "",
      searched: false,
    };
  }
  ...
  const vídeo = Array.isArray(results?.vídeos) ? results.vídeos.find((item) => item?.url) : null;

  if (!vídeo?.url) throw new Erro("No encontré resultados en YouTube.");

  return {
    url: vídeo.url,
    title: cleanText(vídeo.title || "YouTube Vídeo"),
    duration: cleanText(vídeo.timestamp || ""),
    author: cleanText(vídeo.author?.name || vídeo.author || ""),
    thumbnail: cleanText(vídeo.thumbnail || ""),
    vídeoId: cleanText(vídeo.vídeoId || ""),
    searched: true,
  };
}

async function getYtmp4dlData(vídeoUrl, quality) {
  ...
  return {
    remoteUrl,
    title: cleanText(data.title || "YouTube Vídeo"),
    fileName: normalizeMp4Name(data.filename || data.title || "youtube-vídeo.mp4"),
...
async function downloadRemoteMp4(remoteUrl, preferredName) {
  ...
  if (contentLength > MAX_VIDEO_BYTES) {
    throw new Erro(`El vídeo pesa ${humanBytes(contentLength)} y supera el limite del bot.`);
  }
  ...
  response.data.on("data", (chunk) => {
    ...
    if (downloaded > MAX_VIDEO_BYTES) {
      response.data.destroy(new Erro("El vídeo es demais grande para enviarlo por WhatsApp."));
    }
  });
  ...
  if (!stat?.size || stat.size < MIN_VIDEO_BYTES) {
    await deleteFileSafe(outputPath);
    throw new Erro("El archivo MP4 descarregado es inválido.");
  }

  return {
    tempPath: outputPath,
    fileName: normalizeMp4Name(preferredName || "youtube-vídeo.mp4"),
    size: stat.size,
    contentType: "vídeo/mp4",
  };
}
...
function buildResultCaption(data, deliveryLabel) {
  ...
  return [
    "🎬 *YTMP4DL*",
    `• *Vídeo:* ${clipText(data.title || data.fileName || "YouTube Vídeo", 80)}`,
    ...
    requestedLabel !== deliveredLabel ? "• *Nota:* se ajustó a una calidad menor compatível." : null,
...
function buildQualityRows(prefix, resolved, baseQuery = "") {
  ...
  return QUALITY_OPTIONS.map((quality, index) => ({
    ...
    description: clipText(
      `${resolved.title || "YouTube Vídeo"} | ${resolved.duration || "N/D"} | baja sola si falla`,
      72
    ),
...
async function sendQualityPicker(sock, from, quoted, ctx, resolved, rawQuery) {
  ...
  const text = [
    "🎬 *YTMP4DL*",
    `• *Vídeo:* ${clipText(resolved.title || "YouTube Vídeo", 72)}`,
  ...
  const interactivePayload = {
    text,
    title: "🎬 YTMP4DL",
    subtitle: "Escolha una calidad",
    footer: "DVYER API",
    interactiveButtons: [
      {
        name: "single_select",
        buttonParamsJson: JSON.stringify({
          title: "Escolher calidad",
          sections: [
            {
    ...
    try {
      ...
      if (!limitState.ok) {
        return await sock.sendMessage(
          from,
          {
            text: `⚠️ Mucho uso de ytmp4dl. Retente en ${formatRetrySeconds(limitState.retryAfterMs)}s.`,
            ...global.channelInfo,
          },
          quoted
        );
      ...
      if (!apiData) {
        throw lastErro || new Erro("No se pudo obtener el vídeo.");
      }
      ...
      if (!sentSuccessfully) {
        const shownErro =
          erro?.code === "PROVIDER_CIRCUIT_OPEN"
            ? String(erro?.message || "Servicio temporalmente no autorizado para vídeo.")
            : cleanVídeoErroText(erro, "No se pudo preparar el MP4.");
