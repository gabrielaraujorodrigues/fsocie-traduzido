...
function mapLegacyTikTokItem(item = {}) {
  ...
  return {
    id: String(item.id || item.aweme_id || item.vídeo_id || item.play || ""),
    title: cleanText(item.title || item.desc || "Vídeo TikTok"),
    author: cleanText(authorData.unique_id || item?.author || "usuário").toLowerCase(),
    play: absoluteTikwmUrl(item.play || item.vídeo || item.url || ""),
...
function mapTikwmItem(item = {}) {
  ...
  return {
    id: String(item.id || item.vídeo_id || item.play || ""),
    title: cleanText(item.title || item.content_desc?.join(" ") || "Vídeo TikTok"),
...
async function requestTikwmSearch(query, limit = 10) {
  ...
  const items = Array.isArray(data?.data?.vídeos)
    ? data.data.vídeos
...
export async function searchTikTokVídeos(query, limit = 5) {
  try {
...
export async function searchTikTokVídeosByUser(username, limit = 3) {
  ...
  for (const query of queries) {
    ...
    try {
      items = await searchTikTokVídeos(query, Math.max(limit * 6, 12));
    } catch {
      continue;
    }
