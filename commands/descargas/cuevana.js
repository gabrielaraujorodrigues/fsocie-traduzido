...
async function downloadVídeo(url, title = "cuevana") {
  ...
  if (!size || size < MIN_FILE_BYTES) {
    ...
    throw new Erro("El archivo descarregado es invalido.");
  }
...
async function sendVídeoMessage(sock, from, quoted, info) {
...
function buildServerRows(downloadsAll, directUrl, prefix) {
  ...
  if (directUrl) {
    rows.push({
      ...
      description: "Servidor mas rapidamente",
...
export default {
  ...
  category: "download",
  description: "Busca en Cuevana y mostra botones para downloadr",
  ...
  async run(ctx) {
    ...
    if (cmd === "cuevanalink") {
      ...
      await conn.sendMessage(
        from,
        {
          text: "*FSOCIETY BOT*\n\nDownloadndo vídeo, aguarde un momento...",
          ...global.channelInfo,
        },
        quoted
      );
      ...
      let downloaded;
      try {
        downloaded = await downloadVídeo(apiLink, "cuevana-vídeo");
      } catch (erro) {
        console.erro("Cuevana download erro:", erro?.message || erro);
        return conn.sendMessage(
          from,
          {
            text: `No pude downloadr el vídeo.\n${sanitizeProviderMessage(erro, { kind: "vídeo", fallback: "No se pudo downloadr el vídeo." })}`,
            ...global.channelInfo,
          },
          quoted
        );
      ...
      try {
        await sendVídeoMessage(conn, from, quoted, {
          ...downloaded,
          title: "Cuevana",
        });
      } finally {
    ...
    if (cmd === "cuevanadl") {
      ...
      if (!data?.ok) {
        return conn.sendMessage(
          from,
          { text: "No pude obtener el detalhe de Cuevana.", ...global.channelInfo },
          quoted
        );
      }
      ...
      if (!rows.length) {
        return conn.sendMessage(
          from,
          {
            text: "No encontre links de download.",
            ...global.channelInfo,
          },
          quoted
        );
    ...
    const introPayload = thumbBuffer
      ? {
          ...
          caption:
            ...
            `Selecione el contenido que quieres.`,
        }
      : {
        text:
          `*FSOCIETY BOT*\n\n` +
