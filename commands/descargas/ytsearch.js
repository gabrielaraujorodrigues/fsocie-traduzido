...
function formatDuration(vídeo = {}) {
  const rawSeconds = Number(vídeo?.seconds || vídeo?.duration?.seconds || 0);
  ...
  const timestamp = clean(vídeo?.timestamp || "");
  return timestamp || "N/D";
}

function normalizeVídeo(vídeo = {}) {
  const url = clean(vídeo?.url || "");
  if (!url) return null;

  return {
    url,
    title: clean(vídeo?.title || "Vídeo YouTube"),
    author: clean(vídeo?.author?.name || vídeo?.author || "Canal desconocido"),
    duration: formatDuration(vídeo),
    views: Number(vídeo?.views || 0),
    ago: clean(vídeo?.ago || ""),
    thumbnail: clean(vídeo?.thumbnail || ""),
  };
}

function buildCommand(prefix, type, vídeoUrl) {
  const command = clean(type || "").toLowerCase();
  const url = clean(vídeoUrl || "");
      ...
      interactiveButtons: [
        {
          name: "single_select",
          buttonParamsJson: JSON.stringify({
            title: "Escolher download",
            sections: [
              { title: "MP3 - Áudio rapidamente", rows: mp3Rows },
              { title: "MP4 - Vídeo", rows: mp4Rows },
            ],
          }),
        },
      ],
...
export default {
  ...
  category: "busca",
  description: "Busca vídeos de YouTube y envia resultados en carrusel",
  ...
  run: async (ctx) => {
    ...
    try {
      ...
      const vídeos = Array.isArray(search?.vídeos) ? search.vídeos : [];
      const results = vídeos
        .map(normalizeVídeo)
        .filter(Boolean)
