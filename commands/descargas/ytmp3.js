...
function safeFileName(name) {
  return (
    String(name || "youtube-áudio")
      .replace(/[\\/:*?"<>|]/g, "")
      .replace(/[^\w .()[\]-]/g, "")
      .replace(/\s+/g, " ")
      .trim()
      .slice(0, 120) || "youtube-áudio"
  );
}

function normalizeMp3Name(name) {
  const parsed = path.parse(String(name || "").trim());
  const base = safeFileName(parsed.name || name || "youtube-áudio");
  return `${base || "youtube-áudio"}.mp3`;
}
...
async function callYtmp3Api({
  ...
  throw new Erro(
    erros.filter(Boolean).join(" | ") ||
      "El servicio de áudio no respondió correctamente."
  );
}
...
function cleanErroText(erro) {
  ...
  if (
    ...
    return "El servicio de áudio está temporalmente inestable.\nTente otra vez.";
  }
  ...
  if (
    ...
    normalized.includes("youtube esta protegiendo este áudio") ||
    normalized.includes("youtube está protegiendo este áudio") ||
    normalized.includes("protected") ||
    normalized.includes("copyright")
  ) {
    return "Áudio no disponível en este momento.\nPuede estar protegido por YouTube.";
  }
  ...
  if (
    ...
    return "Hay muchas solicitudes. Retente en un momento.";
  }

  if (normalized.includes("403")) {
    return "El link de áudio expiró o fue bloqueado.\nTente otra vez.";
  }

  if (normalized.includes("404")) {
    return "No se encontró el áudio o el link ya no está disponível.";
  }

  if (normalized.includes("timeout")) {
    return "La download tardó demais. Tente otra vez.";
  }
  ...
  if (
    ...
    return "Servicio de áudio temporalmente inestable.\nRetente en un momento.";
  }
...
function scoreUrl(url = "") {
  ...
  if (text.includes("googlevídeo.com")) return 40;
  return 60;
}
...
async function resolveInputToUrl(input) {
  ...
  const vídeo = Array.isArray(results?.vídeos)
    ? results.vídeos.find((item) => item?.url)
    : null;
  ...
  return {
    url: vídeo.url,
    title: cleanText(vídeo.title || "YouTube MP3"),
    thumbnail: cleanText(vídeo.thumbnail || ""),
    duration: Number(vídeo.seconds || 0),
    author: cleanText(vídeo.author?.name || ""),
    searched: true,
  };
}

async function getYtmp3Data(vídeoUrl) {
  ...
  return {
    ...
    fileName: normalizeMp3Name(data.filename || data.title || "youtube-áudio.mp3"),
...
async function requestYtmp3Stream(vídeoUrl) {
...
async function saveResponseToFile(response, outputPath, fallbackName, options = {}) {
  ...
  if (contentLength > maxÁudioBytes) {
    throw new Erro(
      `El MP3 pesa ${humanBytes(contentLength)} y supera el límite permitido (${humanBytes(maxÁudioBytes)}).`
    );
  }
  ...
  if (!stat?.size || stat.size < MIN_AUDIO_BYTES) {
    await deleteFileSafe(outputPath);
    throw new Erro("El archivo MP3 descarregado es inválido.");
  }

  assertDownloadWithinPolicy(options?.ctx || {}, stat.size, "áudios");

  const headerName = parseContentDispositionFileName(
    response.headers?.["content-disposition"]
  );

  const fileName = normalizeMp3Name(headerName || fallbackName || "youtube-áudio.mp3");

  return {
    tempPath: outputPath,
    fileName,
    size: stat.size,
    contentType: response.headers?.["content-type"] || "áudio/mpeg",
  };
}

async function downloadYtmp3File(vídeoUrl, preferredName, knownLinkData = null, options = {}) {
  await ensureTmpDir();
  ...
  const attempts = [
    async () => {
      if (!knownLinkData?.remoteUrl) {
        throw new Erro("No hay link remoto conocido.");
      }
    ...
    async () => {
      const response = await requestYtmp3Stream(vídeoUrl);
      return await saveResponseToFile(
        response,
        path.join(TMP_DIR, `${Date.now()}-${randomUUID()}-ytmp3.mp3`),
        preferredName,
    ...
    async () => {
      const linkData = await getYtmp3Data(vídeoUrl);
...
function buildLimitMessage(retryMs) {
  return [
    ...
    "┃ para volver a downloadr música.",
    "┃",
    "┣━━━〔 AVISO 〕━━━⬣",
    "┃ ✦ Evita spam masivo",
    "┃ ✦ Mantén estable el sistema",
    "┃ ✦ Use el comando con calma",
    "┃",
    "╰━━━〔 FSOCIETY PROTECTION 〕━━━⬣",
  ].join("\n");
}
...
async function sendÁudioPreview(sock, from, quoted, data = {}) {
...
export default {
  command: ["ytmp3", "yta", "ytáudio"],
  categoria: "download",
  category: "download",
  description: "Download áudio MP3 de YouTube con portada previa",
  ...
  run: async (ctx) => {
    ...
    try {
      ...
      await sendÁudioPreview(sock, from, quoted, finalData);
      ...
      const erroText =
        erro?.code === "PROVIDER_CIRCUIT_OPEN"
          ? "Servicio temporalmente no disponível para áudio.\nTente otra vez."
          : cleanErroText(erro);
