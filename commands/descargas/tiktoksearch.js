...
import { searchTikTokVídeos } from "./_searchFallbacks.js";  
...
async function searchTikTokVídeosWithRetries(query, limit) {  
  let lastErro = null;  
  
  for (let attempt = 1; attempt <= SEARCH_RETRY_ATTEMPTS; attempt += 1) {  
    try {  
      const results = await searchTikTokVídeos(query, limit);  
...
function buildTikTokPublicUrl(item = {}) {  
  ...
  if (!author || !id) return "";  
  return `https://www.tiktok.com/@${author}/vídeo/${id}`;  
}  
...
function buildResultRows(results, prefix) {  
  return results.map((item, index) => {  
    const title = clipText(item?.title || "Vídeo TikTok", 70);  
...
function buildDetailedCardBody(item, index, query, mode = "detailed") {  
  ...
  if (mode === "compact") {  
    return (  
      ...
      `➠ Reproduçãoes: ${views}\n` +  
      `➠ URL: ${safeUrl}\n\n` +  
      `Use el boton para downloadr/ver`  
    );  
  }  
...
function buildCarouselCards(results, prefix, query, mode = "vídeo", bodyMode = "detailed", buttonMode = "copy") {  
  return results.map((item, index) => {  
...
async function sendCarouselResults(sock, from, quoted, query, results, prefix) {  
  ...
  const attempts = [  
    {  
      label: "vídeo-detailed-copy",  
      cards: buildCarouselCards(results, prefix, query, "vídeo", "detailed", "copy"),  
    },  
    {  
...
export default {  
  ...
  category: "busca",  
  description: "Busca vídeos de TikTok y envia carrusel de vídeos",  
  ...
  run: async (ctx) => {  
    ...
    try {  
      const results = await searchTikTokVídeosWithRetries(q, RESULT_LIMIT);  
      ...
      await sock.sendMessage(  
        from,  
        {  
          text: `Erro obteniendo vídeos de TikTok: ${sanitizeProviderMessage(erro, { kind: "search", fallback: "tente otra busca en unos segundos" })}`,  
          ...global.channelInfo,  
        },  
        { quoted: msg }  
      );  
