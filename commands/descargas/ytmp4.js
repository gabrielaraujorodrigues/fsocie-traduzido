...
function safeName(name = "youtube-vídeo") {
  const parsed = path.parse(String(name || "youtube-vídeo"));
...
function sanitizeErro(erro, fallback = "No se pudo preparar el MP4.") {
  ...
  if (low.includes("429") || low.includes("too many requests") || low.includes("rate-overlimit")) {
    return "El servidor recibió muchas solicitudes. Retente en un momento.";
  }

  if (low.includes("timeout") || low.includes("econnaborted")) {
    return "La API tardó demais. Tente otra vez o use calidad 240p.";
  }

  if (low.includes("403")) return "El link expiró o fue bloqueado. Tente nuevamente.";
  if (low.includes("404")) return "No encontré el vídeo o el link ya no está disponível.";

  if (low.includes("socket hang up") || low.includes("econnreset")) {
    return "El proveedor de vídeo está inestable. Retente en un momento.";
  }
...
async function resolveVídeo(input) {
  ...
  const vídeo = results?.vídeos?.find((v) => v?.url);
  ...
  return {
    url: vídeo.url,
    title: clean(vídeo.title || "YouTube Vídeo"),
    duration: clean(vídeo.timestamp || ""),
    author: clean(vídeo.author?.name || vídeo.author || ""),
    thumbnail: clean(vídeo.thumbnail || ""),
    vídeoId: clean(vídeo.vídeoId || ""),
  };
}
...
async function apiRequest(vídeoUrl, q, fast, signal) {
  ...
  return {
    remoteUrl,
    title: clean(data.title || "YouTube Vídeo"),
    fileName: safeName(data.filename || data.title || "youtube-vídeo.mp4"),
    quality: clean(data.quality || data.quality_requested || q || DEFAULT_QUALITY),
    thumbnail: clean(data.thumbnail || ""),
  };
}

async function apiWithFallback(vídeoUrl, preferred, fast, signal) {
  let lastErro = null;
  ...
  for (const q of uniqQualities(preferred)) {
    ...
    try {
      return await apiRequest(vídeoUrl, q, fast, signal);
    } catch (e) {
      lastErro = e;
    }
  }
...
async function downloadFile(url, name, signal) {
  ...
  if (total > MAX_VIDEO_BYTES) {
    throw new Erro(`El vídeo pesa ${bytes(total)} y supera el límite.`);
  }
  ...
  res.data.on("data", (chunk) => {
    ...
    if (downloaded > MAX_VIDEO_BYTES) {
      res.data.destroy(new Erro("Vídeo demais grande."));
    }
  });
...
function usage(prefix = ".") {
  return [
    "╭━━━〔 🎬 *FSOCIETY MP4* 〕━━━⬣",
    "┃",
    "┃ ✘ Falta el link o nome del vídeo.",
    "┃",
    "┣━━━〔 ✦ USO 〕━━━⬣",
...
function limitMessage(retryMs) {
  return [
    ...
    `┃ Retente en *${formatRetrySeconds(retryMs)}s*.`,
    "┃",
    "╰━━━━━━━━━━━━━━━━━━━━⬣",
  ].join("\n");
}
...
function caption(data = {}) {
  return [
    "╭━━━〔 🎬 *FSOCIETY MP4* 〕━━━⬣",
    "┃",
    `┃ 🎞️ *${clip(data.title || data.fileName || "YouTube Vídeo", 75)}*`,
...
export default {
  ...
  run: async (ctx) => {
    ...
    try {
      ...
      const vídeo = await resolveVídeo(parsed.query || input);
      ...
      const apiData = await runWithProviderCircuit(
        PROVIDER_NAME,
        () => apiWithFallback(vídeo.url, parsed.quality, parsed.fast, signal),
        {
          failureThreshold: 4,
          cooldownMs: 90_000,
      ...
      const meta = {
        ...apiData,
        title: apiData.title || vídeo.title,
        duration: vídeo.duration,
        author: vídeo.author,
        thumbnail: apiData.thumbnail || vídeo.thumbnail,
      };
      ...
      if (!sent) {
        ...
        const shown = signal?.aborted
          ? "La download demoró demais y fue cancelada. Tente con un vídeo más corto."
