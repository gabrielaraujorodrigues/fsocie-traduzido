...
function addPlayerToClan(tournament, clanName, { jid = "", nick = "" } = {}) {
  ...
  if (!clan) {
    return { ok: false, message: "No encontre ese clan. Use ffclanes para revisar nomes." };
  }
  ...
  if (current && normalizeClanKey(current.clan.name) !== normalizeClanKey(clan.name)) {
    return {
      ok: false,
      message: `Ya estas inscrito en *${current.clan.name}*. Use ffbaja para cambiarte.`,
    };
  }
...
function createMatch(tournament, clanAName, clanBName, roundLabel = "") {
  ...
  if (!clanA || !clanB) {
    return { ok: false, message: "Ambos clanes deben estar registrados primeiro." };
  }
  ...
  if (missingA > 0 || missingB > 0) {
    const lines = [];
    if (missingA > 0) lines.push(`- ${clanA.name}: faltan ${missingA} jogador(es).`);
    if (missingB > 0) lines.push(`- ${clanB.name}: faltan ${missingB} jogador(es).`);
    return {
      ok: false,
      message:
        ...
        `${lines.join("\n")}\n\n` +
        `Use ffinscribir para completar equipos.`,
    };
  }
...
function applyMatchResult(tournament, matchId, winnerName, scoreText, killsText = "", actor = "") {
  ...
  if (match.status === "jugado" && match.result) {
    return { ok: false, message: "Ese match ya tiene resultado carregado." };
  }

  const score = parseScore(scoreText);
  if (!score) {
    return { ok: false, message: "Formato de score invalido. Use 2-1 o 6:4." };
  }
...
function buildRulesSummary(rules = cloneDefaultFormatRules()) {
  return (
    ...
    `Rondas por mapa: *BO${rules.roundsBestOf}* (primeiro a ${rules.roundsToWin})\n` +
...
function formatPlayerLine(player, idx) {
  ...
  return `${idx + 1}. ${nick} (${number ? `+${number}` : "sin número"})`;
}
...
function updateTournamentFormat(tournament, textInput) {
  ...
  if (teamPattern) {
    ...
    if (!Number.isFinite(left) || !Number.isFinite(right) || left <= 0 || right <= 0) {
      return { ok: false, message: "Formato de equipos invalido. Use por ejemplo 4v4 o 5v5." };
    }
    if (left !== right) {
  ...
  if (teamAlt) {
    ...
    if (!Number.isFinite(teamSize) || teamSize <= 0 || teamSize > 12) {
      return { ok: false, message: "Cantidad de jogadores invalida. Use un valor entre 1 y 12." };
    }
...
function buildUsage(prefix = ".") {
  return (
    `*FREE FIRE TORNEOS*\n\n` +
    `Catalogo rapidamente:\n` +
    `- ${prefix}ffcatalogo\n\n` +
    `Crear torneo (admin):\n` +
    `- ${prefix}ffcrear Nome del torneo\n\n` +
    ...
    `  (último valor = killsA-killsB, opçãoal)\n\n` +
    `Ver estado:\n` +
    `- ${prefix}ffestado\n` +
    `- ${prefix}ffpartidos\n` +
    `- ${prefix}fftabla\n\n` +
...
export default {
  ...
  run: async ({
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `Crea uno con: *${prefix}ffcrear Nome del torneo*`,
          ...global.channelInfo,
        },
        { quoted: msg }
      );
    ...
    if (action === "clan") {
      if (!requireAdmin({ esAdmin, esDono })) {
        return sock.sendMessage(from, { text: "Solo admin/owner puede gestãoar clanes.", ...global.channelInfo }, { quoted: msg });
      }
    ...
    if (action === "inscritos") {
      ...
      if (clanName) {
        ...
        if (!clan) {
          return sock.sendMessage(
            from,
            { text: "No encontre ese clan. Use ffclanes para revisar el nome.", ...global.channelInfo },
            { quoted: msg }
          );
        }
