...
export default {
  ...
  category: "jogos",
  description: "Responda si una afirmacion es verdadera o falsa",
  ...
  onMessage: async ({ sock, msg, from, sender, text, settings }) => {
    ...
    if (answer === null) {
      await sock.sendMessage(
        from,
        {
          text: "Responda solo con verdadero, falso, v o f.",
          ...global.channelInfo,
        },
        { quoted: msg }
      );
    ...
    await sock.sendMessage(
      from,
      {
        text:
          ...
          `Afirmacion: *${session.statement}*\n` +
          `Resposta correta: *${session.answer ? "VERDADERO" : "FALSO"}*\n` +
          `${session.explanation ? `Dato: ${session.explanation}` : ""}`,
        ...global.channelInfo,
      },
      { quoted: msg }
