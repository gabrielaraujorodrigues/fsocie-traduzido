      ...
      const lines = team.length
        ? team
            .map((player, row) => {
              const number = normalizeText(player.number) || numberFromJid(player.jid);
              const nick = normalizeName(player.nick || "Jogador");
              return `   ${row + 1}. ${nick} (${number ? `+${number}` : "sin número"})`;
            })
            .join("\n")
        : "   Sin jogadores";
      return `*Equipo ${idx + 1}*\n${lines}`;
...
function resolveTemplate(parts = []) {
  ...
  const base = template || {
    title: normalizeText(parts[0]) || "Evento Comunidade",
    game: "Comunidade",
    format: "4v4",
    slots: 20,
    mode: "general",
...
function createEvent(groupId, payload = {}, createdBy = "") {
  ...
  const event = {
    id,
    title: normalizeText(payload.title) || "Evento Comunidade",
    game: normalizeText(payload.game) || "Comunidade",
...
async function sendEventoPainel({ sock, from, msg, settings, event = null }) {
  const prefix = getPrimaryPrefix(settings);
  const summary = event
    ? formatEventCard(event)
    : "No hay evento ativo en este grupo.\nCrea uno desde el painel.";
      ...
      rows: [
        {
          ...
          description: "Registro automático con tu número.",
      ...
      rows: [
        {
          ...
          description: "Resumo del torneo ativo.",
...
export default {
  ...
  run: async ({
    ...
    if (action === "menu") {
      return sendEventoPainel({ sock, from, msg, settings, event });
    }
        ...
        return sock.sendMessage(
          from,
          {
            text:
              ...
              `Use *${getPrimaryPrefix(settings)}evento finalizar* para cerrar el actual.`,
            ...global.channelInfo,
          },
          { quoted: msg }
        );
    ...
    if (action === "finalizar" || action === "cerrarevento" || action === "reset") {
      ...
      return sock.sendMessage(
        from,
        {
          text: `🏁 Evento finalizado: *${event.title}*.\nPuedes crear uno novo con *${getPrimaryPrefix(settings)}evento crear ff*.`,
          ...global.channelInfo,
        },
        { quoted: msg }
      );
    ...
    return sock.sendMessage(
      from,
      {
        text:
          ...
          `Use *${getPrimaryPrefix(settings)}evento* para abrir el painel.`,
        ...global.channelInfo,
      },
      { quoted: msg }
    );
