...
export default {
  name: "banderas",
  command: ["banderas", "flagquiz", "adivinhebandera"],
  category: "jogos",
  description: "Adivinhe el pais segun su bandera",
  ...
  run: async ({ sock, msg, from, sender, settings }) => {
    ...
    return sock.sendMessage(
      from,
      {
        text:
          `*QUIZ BANDERAS*\n\n` +
          `Que pais corresponda a esta bandera?\n` +
          `${item.flag}\n\n` +
          `Tentativas: *0/${MAX_ATTEMPTS}*\n` +
          `Use *${prefix}salirjogo* para cancelar.`,
        ...global.channelInfo,
      },
      { quoted: msg }
    );
      ...
      await sock.sendMessage(
        from,
        {
          text:
            ...
            `${session.flag}\n` +
            `Resposta correta: *${session.country.toUpperCase()}*`,
          ...global.channelInfo,
        },
        { quoted: msg }
      );
