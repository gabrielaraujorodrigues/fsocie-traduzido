...
const WEEKLY_MISSIONS = Object.freeze([
  ...
  },
  {
    ...
    description: "Consume 8 solicitudes de download esta semana.",
    metric: "requests",
    target: 8,
...
const ACHIEVEMENTS = Object.freeze([
  ...
  {
    id: "ach_req_100",
    title: "Descarregador elite",
    description: "Use 100 solicitudes de download.",
    metric: "requests",
    target: 100,
...
const PREMIUM_SHOP = Object.freeze([
  ...
  },
  {
    ...
    description: "Recibe 10 solicitudes extra de download.",
    levelRequired: 2,
    price: 700,
    grant: { requests: 10 },
  },
  ...
  },
  {
    id: "tag_diamante",
    name: "Tag Diamante",
    description: "Tag premium de alto nível.",
    levelRequired: 5,
    price: 2600,
  ...
  },
  {
    id: "caja_elite",
    name: "Caja Elite",
    description: "Caja de coleccion nível alto.",
    levelRequired: 7,
    price: 3900,
...
function claimPassTierReward({ sender, settings, user, tierNumber, premiumTrack = false, chatId }) {
  ...
  if (claimedBucket?.[claimKey]) {
    return { ok: false, message: "Ese prêmio del pase ya fue reclamado." };
  }
...
function buyPremiumItem({ sender, settings, user, itemId }) {
  ...
  if (user.level < item.levelRequired) {
    return { ok: false, message: `Necesitas nível premium *${item.levelRequired}* para comprar *${item.id}*.` };
  }
...
function formatMissionRow(row) {
  ...
  return (
    ...
    `Prêmio: *${rewardParts(row.reward).join(" | ")}*`
  );
}
...
function formatShopRow(item) {
  return (
    `*${item.id}* (Nível ${item.levelRequired}+)\n` +
    `${item.name}\n` +
    `${item.description}\n` +
    `Preço: *${formatCoins(item.price)}*`
  );
}

async function sendPremiumPainel({ sock, from, msg, settings, profile, user }) {
          ...
          sections: [
              ...
              rows: [
                {
                  ...
                  description: "Nível, XP, pass y racha.",
            ...
            },
            {
              ...
              rows: [
                ...
                { header: "ACTIVIDAD", title: "Reclamar actividad", description: "Prêmio por uso de comandos", id: `${prefix}premium reclamar actividad` },
              ],
            },
            {
...
export default {
  ...
  run: async ({ sock, msg, from, sender, args = [], settings = {}, commandName = "" }) => {
    ...
    if (!action || ["painel", "menu", "help", "inicio"].includes(action)) {
      return sendPremiumPainel({ sock, from, msg, settings, profile, user });
    }
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `Estado: *${user.season.pass.owned ? "Premium ativo ✅" : "Solo free ⚪"}*\n` +
            `Pass XP: *${user.season.pass.xp}*\n\n` +
            `${passRows.map((row) => formatPassRow(row)).join("\n\n")}\n\n` +
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `Último reclamo: *${user.streak.lastClaimDay || "Nunca"}*\n` +
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `Próximo hito: *${result.nextMilestone} comandos*`,
          ...global.channelInfo,
        },
        { quoted: msg }
      );
    ...
    return sock.sendMessage(
      from,
      {
        text:
          `Accion no valida.\n` +
          `Use *${prefix}premium* para abrir el painel premium.`,
        ...global.channelInfo,
      },
      { quoted: msg }
    );
