...
export default {
  ...
  category: "economia",
  description: "Compra solicitudes extra de download con tus dolares",
  ...
  run: async ({ sock, msg, from, sender, args = [], settings }) => {
    ...
    if (!result.ok) {
      ...
      if (result.status === "invalid_amount") {
        text = `Use una cantidad valida. Ejemplo: *${prefix}buyrequests 5*`;
    ...
    await sock.sendMessage(
      from,
      {
        text:
          ...
          `Solicitudes disponíveis: *${result.requests?.available || 0}*`,
        ...global.channelInfo,
      },
      { quoted: msg }
    );
