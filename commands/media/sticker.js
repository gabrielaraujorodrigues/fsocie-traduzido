...
export default {
  command: ["figurinha", "s"],
  category: "media",
  description: "Imagem/Vídeo a figurinha",
  ...
  run: async ({ sock, msg, from }) => {
    try {
      ...
      const hasVídeo =
        !!targetMsg.message?.vídeoMessage ||
        !!targetMsg.message?.extendedTextMessage?.contextInfo?.quotedMessage?.vídeoMessage;

      if (!hasImage && !hasVídeo) {
        return sock.sendMessage(
          from,
          { text: "⚙️ Responda a una *imagem/vídeo* con .figurinha", ...global.channelInfo },
          { quoted: msg }
        );
      }
      ...
      const inFile = path.join(TMP_DIR, randName(hasVídeo ? "mp4" : "jpg"));
      ...
      return sock.sendMessage(from, { figurinha: webp, ...global.channelInfo }, { quoted: msg });
    } catch (e) {
      console.erro("figurinha erro:", e);

      // Si tu server no tiene ffmpeg instalado, este es el erro típico
      ...
      return sock.sendMessage(
        from,
        { text: `❌ Erro creando figurinha.${tip}`, ...global.channelInfo },
        { quoted: msg }
      );
    }
  }
