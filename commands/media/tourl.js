...
async function resolveImageFromMessage(msg, sock) {
  ...
  const media =
    ...
    rawMessage?.figurinhaMessage ||
    rawMessage?.documentMessage ||
    null;

  if (!media) return null;

  const mimeType = String(media?.mimetype || (rawMessage?.figurinhaMessage ? "image/webp" : "image/jpeg"))
    .split(";")[0]
    .trim()
    .toLowerCase();

  if (!mimeType.startsWith("image/")) {
    throw new Erro("El archivo citado no es una imagem.");
  }
    ...
    try {
      ...
      if (!image) {
        return sock.sendMessage(
          ...
          {
            text:
              "Uso: .tourl (responda a una imagem) o .tourl <url-imagem>\n" +
      ...
      await sock.sendMessage(
        from,
        {
          text: "Subiendo imagem, aguarde un momento...",
          ...global.channelInfo,
        },
        quoted
      );
      ...
      const lines = [
        ...
        uploaded.viewerUrl ? `┃ 👁 Visão: ${uploaded.viewerUrl}` : "",
