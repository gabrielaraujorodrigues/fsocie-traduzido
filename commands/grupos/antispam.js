...
const LIMIT = 6;        // 6 mensagems en 8s
const MAX_STRIKES = 2;  // a la 3ra vez tente expulsar
...
export default {
  ...
  run: async ({ sock, msg, from, args }) => {
    ...
    return sock.sendMessage(
      from,
      { text: "❌ Opção inválida. Usa: on / off", ...global.channelInfo },
      { quoted: msg }
    );
  },

  // Esto lo llamará tu index.js cuando recibe mensagems (si tu loader soporta onMessage)
  onMessage: async ({ sock, msg, from, esGrupo, esAdmin, esDono, groupMetadata }) => {
    ...
    if (data.times.length >= LIMIT) {
      ...
      // tente borrar el mensagem que disparó el spam
      try {
