...
export default {
  ...
  groupOnly: true,
  adminOnly: true,
  description: "Reenvia mensagems excluídos en grupos",
  ...
  async run({ sock, msg, from, args = [] }) {
    ...
    return sock.sendMessage(
      from,
      {
        text: "Use .antidelete on o .antidelete off",
        ...global.channelInfo,
      },
      quoted
    );
  ...
  async onMessageDelete({ sock, from, isGroup, deleteKey, deletedMessage }) {
    ...
    if (recoveredText) {
      ...
      body += `\n\nTipo: imagem`;
    } else if (deletedMessage?.message?.vídeoMessage) {
      body += `\n\nTipo: vídeo`;
    } else if (deletedMessage?.message?.áudioMessage) {
      body += `\n\nTipo: áudio`;
