...
function toAction(value = "") {
  ...
  if (["on", "encender", "ligar", "ativar", "enable", "1", "si"].includes(normalized)) {
    return "on";
  }

  if (["off", "desligar", "desativar", "disable", "0", "no"].includes(normalized)) {
    return "off";
  }
...
function buildPainelMessage(prefix, isOff) {
  const statusText = isOff ? "OFF 🔴" : "ON 🟢";
  const modeText = isOff
    ? "Ahora el bot no respondara comandos en este grupo."
    : "El bot respondara normalmente en este grupo.";
...
function buildInteractiveRows(prefix) {
  return [
    ...
    },
    {
      ...
      description: "Mostra si el bot esta ON u OFF en este grupo.",
...
export default {
  ...
  run: async ({ sock, msg, from, args = [], settings, commandName = "" }) => {
    ...
    if (!action) {
      return sock.sendMessage(
        from,
        {
          text: buildPainelMessage(prefix, isOff),
          title: "FSOCIETY BOT",
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `El bot vuelve a respondar comandos normalmente.`,
          ...global.channelInfo,
        },
        quoted
      );
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `Desde ahora no respondare comandos aqui.\n` +
  ...
  onMessage: async ({ sock, msg, from, esGrupo, esAdmin, esDono, text, settings, comandos, groupMetadata }) => {
    ...
    // Usuário normal: bloquear en silencio (sin avisos).
    if (!esAdmin && !esDono) {
      return true;
    }
