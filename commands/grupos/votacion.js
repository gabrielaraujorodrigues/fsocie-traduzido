...
export default {
  ...
  category: "grupo",
  description: "Votações de grupo con cierre automático",
  groupOnly: true,
  ...
  run: async ({ sock, msg, from, sender, args = [], settings, esAdmin = false, esDono = false, commandName = "" }) => {
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `- ${prefix}votar 1\n` +
            `- ${prefix}votação estado\n` +
            `- ${prefix}votação cerrar (admin)\n` +
    ...
    if (action === "crear") {
      if (!(esAdmin || esDono)) {
        return sock.sendMessage(from, { text: "Solo admin/owner puede crear votaçãoes.", ...global.channelInfo }, { quoted: msg });
      }
      if (activePoll(from)) {
      ...
      if (!input) {
        return sock.sendMessage(
          from,
          {
            text:
              `Uso:\n` +
              `- ${prefix}votação crear 10m | Elegimos hora | 8PM | 9PM\n` +
              `- ${prefix}votação crear Reunion | 30m | Hoy | Manana`,
            ...global.channelInfo,
          },
          { quoted: msg }
        );
    ...
    if (action === "votar" || action === "vote") {
      ...
      if (index < 0) {
        return sock.sendMessage(from, { text: "Opção inválida para esta votação.", ...global.channelInfo }, { quoted: msg });
      }
    ...
    if (action === "estado" || action === "resultados" || action === "resumo") {
    ...
    if (action === "cerrar") {
      if (!(esAdmin || esDono)) {
        return sock.sendMessage(from, { text: "Solo admin/owner puede cerrar votaçãoes.", ...global.channelInfo }, { quoted: msg });
      }
    ...
    if (action === "cancelar") {
      if (!(esAdmin || esDono)) {
        return sock.sendMessage(from, { text: "Solo admin/owner puede cancelar votaçãoes.", ...global.channelInfo }, { quoted: msg });
      }
    ...
    return sock.sendMessage(
      from,
      { text: `Accion invalida. Use *${prefix}votação*`, ...global.channelInfo },
      { quoted: msg }
    );
  },
