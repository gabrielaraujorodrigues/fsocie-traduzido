...
export default {
  ...
  command: ["antiflood"],
  category: "grupo",
  description: "Controla flood de mensagems en grupos",
  groupOnly: true,
  adminOnly: true,
  ...
  run: async ({ sock, msg, from, args = [] }) => {
    ...
    if (action === "config") {
      ...
      store.scheduleSave();
      return sock.sendMessage(from, { text: `Antiflood atualizado a ${config.limit} mensagems / ${config.windowSeconds}s`, ...global.channelInfo }, { quoted: msg });
    }
  ...
  onMessage: async ({ sock, msg, from, sender, esGrupo, esAdmin, esDono, groupMetadata }) => {
    ...
    await sock.sendMessage(
      from,
      {
        text: `Antiflood: ${getParticipantDisplayTag(null, sender)} baja la velocidade de mensagems.`,
