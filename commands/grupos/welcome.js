...
function buildStatusText(config, prefix) {
  ...
  return (
    ...
    `Saída: *${boolLabel(config.byeEnabled)}*\n` +
    `Imagem: *${imageLabel}*\n` +
    ...
    `- ${prefix}welcome text Bem-vindo @user a @group\n` +
...
function buildWelcomeMessage({
  ...
  const mainText =
    customText || `Bem-vindo @user a @group. Aguardemos que disfrutes tu estancia.`;
...
function buildByeMessage({
  ...
  const mainText = byeText || `Até logo @user. Te aguardemos de vuelta en @group.`;
  const renderedMainText = renderTemplate(mainText, {
    userTag,
    groupName,
    totalMembers,
...
function buildCardText(action, config, userTag, groupName, totalMembers, botName) {
  if (action === "remove") {
    return renderTemplate(
      config.byeText || `Até logo @user. Te aguardemos de vuelta en @group.`,
      {
        userTag,
        groupName,
        totalMembers,
  ...
  return renderTemplate(
    config.text || `Bem-vindo @user a @group. Aguardemos que disfrutes tu estancia.`,
    {
      userTag,
      groupName,
      totalMembers,
  ...
  async run({ sock, from, args = [], msg, settings }) {
    ...
    if (!args.length || ["status", "estado", "menu"].includes(action)) {
      return sock.sendMessage(
        from,
        {
          ...
          title: "FSOCIETY BOT",
          subtitle: "Painel Boas-vindas & Saída",
          footer: "Selecione uma opção",
          interactiveButtons: [
            {
              name: "single_select",
              buttonParamsJson: JSON.stringify({
                title: "Configurar Boas-vindas/Saída",
                sections: [
                  {
                    title: "Boas-vindas",
                    rows: [
                      {
                        ...
                        description: "Envia mensagem cuando entra alguém.",
                        id: `${prefix}welcome on`,
                      },
                      {
                        header: "WELCOME OFF",
                        title: "Desativar boas-vindas",
                        description: "Apaga mensagem de entrada.",
                  ...
                  },
                  {
                    ...
                    rows: [
                      {
                        header: "BYE ON",
                        title: "Ativar mensagem de saída",
                        description: "Envia mensagem cuando alguém sale.",
                        id: `${prefix}welcome bye on`,
                      },
                      {
                        header: "BYE OFF",
                        title: "Desativar mensagem de saída",
                        description: "Apaga mensagem de salida.",
                  ...
                  },
                  {
                    ...
                    rows: [
                      {
                        header: "STATUS",
                        title: "Ver estado atual",
                        description: "Mostra configuração de welcome/bye.",
    ...
    if (action === "rules") {
      ...
      return sock.sendMessage(
        from,
        {
          text: "✅ Regras de boas-vindas actualizadas.",
          ...global.channelInfo,
        },
        quoted
      );
    ...
    if (action === "image") {
      ...
      return sock.sendMessage(
        from,
        {
          text: "✅ Imagem salva (se use como respaldo si falla la foto del grupo).",
          ...global.channelInfo,
        },
        quoted
      );
    ...
    if (action === "reset") {
      ...
      return sock.sendMessage(
        from,
        {
          text: "✅ Boas-vindas & Saída reiniciados.",
          ...global.channelInfo,
        },
        quoted
      );
