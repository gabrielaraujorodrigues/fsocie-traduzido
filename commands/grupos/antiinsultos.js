...
export default {
  ...
  category: "grupo",
  description: "Anti-insultos: 3 avisos y expulsión (solo admins)",
  groupOnly: true,
  adminOnly: true,
  ...
  onMessage: async ({ sock, msg, from, esGrupo, esAdmin, esDono, groupMetadata }) => {
    ...
    if (current >= MAX_WARNS) {
      ...
      // Si NO pudo expulsar, NO borramos: se queda en 3 para intentar de novo
      return sock.sendMessage(from, {
        text:
          `🚫 *ANTI-INSULTOS*\n` +
          `${getParticipantDisplayTag(null, sender)} llegó a *${MAX_WARNS}/${MAX_WARNS}* avisos.\n` +
