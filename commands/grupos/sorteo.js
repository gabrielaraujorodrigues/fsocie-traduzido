...
function createSorteio(groupId, payload = {}, sender = "") {
  ...
  state.active = {
    id,
    prize: cleanText(payload.prize) || "Prêmio sorpresa",
...
export default {
  ...
  category: "grupo",
  description: "Sorteios en grupo con inscripcion y cierre automático",
  groupOnly: true,
  ...
  run: async ({ sock, msg, from, sender, args = [], settings, esAdmin = false, esDono = false, commandName = "" }) => {
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `- ${prefix}sorteio salir\n` +
            `- ${prefix}sorteio estado\n` +
            `- ${prefix}sorteio cerrar (admin)\n` +
    ...
    if (action === "crear") {
      if (!(esAdmin || esDono)) {
        return sock.sendMessage(from, { text: "Solo admin/owner puede crear sorteios.", ...global.channelInfo }, { quoted: msg });
      }
      ...
      if (!input) {
        return sock.sendMessage(
          from,
          {
            text:
              `Uso:\n` +
              `- ${prefix}sorteio crear 10m | Nitro Discord\n` +
              `- ${prefix}sorteio crear Prêmio sorpresa | 30m`,
            ...global.channelInfo,
          },
          { quoted: msg }
        );
    ...
    if (action === "salir" || action === "leave") {
      ...
      return sock.sendMessage(
        from,
        {
          text: `Listo, saliste del sorteio.\nParticipantes: *${active.participants.length}*`,
          ...global.channelInfo,
        },
        { quoted: msg }
      );
    ...
    if (action === "cerrar") {
      if (!(esAdmin || esDono)) {
        return sock.sendMessage(from, { text: "Solo admin/owner puede cerrar sorteios.", ...global.channelInfo }, { quoted: msg });
      }
    ...
    if (action === "cancelar") {
      if (!(esAdmin || esDono)) {
        return sock.sendMessage(from, { text: "Solo admin/owner puede cancelar sorteios.", ...global.channelInfo }, { quoted: msg });
      }
    ...
    return sock.sendMessage(
      from,
      { text: `Accion invalida. Use *${prefix}sorteio*`, ...global.channelInfo },
      { quoted: msg }
    );
  },
