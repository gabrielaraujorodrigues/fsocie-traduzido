...
function isDono(metadata = {}, participant = {}) {
  const owner = cleanText(metadata?.owner || metadata?.subjectDono || metadata?.descDono).toLowerCase();
...
function participantRole(metadata = {}, participant = {}) {
  if (isDono(metadata, participant)) return "owner";
...
export default {
  command: ["tagall", "marcar todos", "marcar todostodos", "llamartodos", "marquertodos", "todos"],
  category: "grupo",
  description: "Invoca y etiqueta a todos los membros del grupo",
  groupOnly: true,
  adminOnly: true,
