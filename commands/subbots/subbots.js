...
function buildDonoSubbotSummary(bot = {}) {
  ...
  return (
    ...
    `Estado: ${state}\n` +
    (waNumber || waName
      ? `WhatsApp: ${waName || "Sin nome"} | ${waNumber || "Sin número"}\n`
      : "") +
...
function buildDonoInteractiveSections(bots = [], prefix = ".") {
  ...
  const slotRows = managed.map((bot) => {
    ...
    return {
      ...
      description: `Nro: ${assignedNumber || "sin número"} | ${state}`.slice(0, 72),
...
export default {
  ...
  category: "subbots",
  description: "Mostra el painel de subbots",
  ...
  run: async ({
    ...
    const ownerView = hasDonoViewAccess(action, commandName);
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `${okLines}` +
            `${badLines}\n\n` +
            `WhatsApp > Dispositivos vinculados > Vincular con número de telefono.`,
          ...global.channelInfo,
        },
        quoted
      );
    ...
    if (ownerView) {
      if (!esDono) {
        return sock.sendMessage(
          from,
          {
            text: "Solo el owner puede abrir el painel privado de subbots.",
            ...global.channelInfo,
          },
          quoted
        );
      ...
      const sections = buildDonoInteractiveSections(ownerBots, prefix);
      ...
      try {
        if (sections.length) {
          await sock.sendMessage(
            ...
            {
              text:
                `Painel owner de subbots.\n` +
                `Conectados: *${activeCount}* | Vinculados: *${linkedCount}* | En aguarde: *${waitingCount}*\n` +
                `Visão: ${chatStatus}\n` +
                `Toca un número/slot para abrir su menu privado.`,
              title: "SUBBOTS OWNER",
              subtitle: "Gestão privada",
              footer: "FSOCIETY BOT",
              interactiveButtons: [
                {
                  name: "single_select",
        ...
        console.erro("No pude enviar menu owner de subbots:", erro?.message || erro);
      }
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `Livres: *${subbotAccess.availableSlots}*\n` +
            `Ativos: *${activeCount}*\n` +
            `En aguarde: *${waitingCount}*\n` +
