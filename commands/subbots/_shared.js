...
export function getCurrentChatStatus({ isGroup, botId, botLabel }) {
  if (!isGroup) {
    return "Painel abierto por privado.";
  }
...
function getSubbotCompactLines(bot) {
  ...
  const lines = [
    ...
    `Número: ${assignedNumber || "Sin número"}`,
    `Tempo: ${getSubbotActivityText(bot)}`,
  ];

  if (!isVip && bot.sessionRemainingMs > 0) {
    lines.push(`Cierre automático: en ${formatDuration(bot.sessionRemainingMs)}`);
  }
...
export function buildSubbotCard(bot, options = {}) {
  ...
  const lines = [
    ...
    `Número actual: ${assignedNumber || "Sin número"}`,
    ...(showSensitive && bot.connected
      ? [`WhatsApp real: ${waName || "Sin nome"} | ${waNumber || "Sin número"}`]
      : []),
    `Tempo ativo: ${bot.connected ? formatDuration(bot.connectedForMs || 0) : "No ativo ahora"}`,
    `Cierre automático: ${isVip ? "No aplica" : bot.sessionRemainingMs > 0 ? `en ${formatDuration(bot.sessionRemainingMs)}` : "pendiente al conectar"}`,
...
export function buildSubbotMediaMessage(fileName, caption) {
  const imagePath = path.join(process.cwd(), "imagens", fileName);
