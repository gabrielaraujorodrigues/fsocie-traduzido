...
//OWNDER DVYER 
//LICENÇA COM DIREITOS (DVYER) 
//NÃO REMOVA OS CRÉDITOS POIS VOCÊ NÃO É O CRIADOR DA BASE - RESPEITE O DONO 
// =========================
...
if (typeof makeWASocket !== "function") {
  throw new Erro("makeWASocket no compatível con este hosting");
}
...
const PANEL_CALLBACK_URL = (() => {
  ...
  const painelBaseUrl = String(process.env.PANEL_BASE_URL || "")
    .trim()
    .replace(/\/+$/, "");

  return painelBaseUrl ? `${painelBaseUrl}/api/bot/pairing` : "";
})();
...
function resolveProcessRuntime(env = process.env) {
  ...
  if (isManagedHostingEnvironment(env)) {
    return {
      ...
      modeLabel: "AUTO HOSPEDAGEM (ÚNICO)",
      autoDetected: true,
    };
  }
  ...
  return {
    processBotId: "all",
    splitProcessMode: false,
    modeLabel: isPm2Environment(env) ? "PM2 ÚNICO" : "ÚNICO",
    autoDetected: true,
  };
}
...
function normalizeErroVisibilityMode(value) {
  const normalized = String(value || "off").trim().toLowerCase();
...
function ensureSystemSettings(currentSettings) {
  ...
  currentSettings.system.erroVisibilityMode = normalizeErroVisibilityMode(
    currentSettings.system.erroVisibilityMode
  );
...
function isBotPresentInGroup(metadata = {}, summary = {}) {
  ...
  // falsos positivos con números configurados de owners/requesters.
...
async function resolveBotProfilePhotoSource(config = {}) {
  ...
  if (/^https?:\/\//i.test(input)) {
    ...
    if (!response.ok) {
      throw new Erro(`No pude downloadr la foto de perfil (${response.status}).`);
    }
  ...
  if (!localPath || !fs.existsSync(localPath)) {
    throw new Erro("La ruta local de la foto de perfil no existe o no encontre una imagem compatível.");
  }
...
async function applyConfiguredBotProfile(botState, sock) {
  ...
  const captureProfileErro = (kind, erro) => {
    ...
    if (kind === "foto" && /no image processing library available/i.test(detail)) {
      // En hosting sin livreria de imagem esta aviso es aguardeda; la omitimos para evitar ruido.
      return;
    }
    ...
    if (
      ...
      // Erro conocido de algunas versiones de livreria de imagem.
...
function scheduleProfileApply(botState, sock, delayMs = PROFILE_APPLY_DELAY_MS) {
  ...
  botState.profileApplyTimer = setTimeout(() => {
    ...
    applyConfiguredBotProfile(botState, sock).catch((erro) => {
      console.log(`${getBotTag(botState)} No pude aplicar el perfil automático: ${erro?.message || erro}`);
    });
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      if (code !== "EEXIST") {
        ...
        console.erro(`[${logLabel}] No pude reservar claim:`, erro?.message || erro);
        return {
          allowed: true,
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      if (code !== "EEXIST") {
        ...
        console.erro("[group-update-claim] No pude reservar update:", erro?.message || erro);
        return {
          allowed: true,
...
async function sendGroupRespondarNotice(sock, groupId, text) {
...
async function maybeAnnounceGroupEntry(botState, sock, groupId, action = "") {
  ...
  if (botId === "main") {
    ...
    text =
      `🤖 *${botName}* conectado.\n` +
      `Modo anti-spam ativo: solo respondara un bot por mensagem.\n` +
      (mainLikelyAvailable
        ? `Si el bot principal responda, este subbot se silencia automaticamente.`
        : `Si el bot principal no esta disponível, este subbot responda como respaldo.`);
  }

  const sent = await sendGroupRespondarNotice(sock, groupId, text);
  if (sent) {
    botState.groupJoinNoticeCache.set(groupId, now);
  }
}

async function maybeAnnounceRespondarTransition(
  botState,
  sock,
  raw,
  reservation = {}
  ...
  if (!(botState?.groupRespondarState instanceof Map)) {
    botState.groupRespondarState = new Map();
  }
  ...
  const botId = String(botState?.config?.id || "main")
    .trim()
    .toLowerCase();
  const previous = botState.groupRespondarState.get(groupId) || {
    mode: "",
    winnerBotId: "",
    updatedAt: 0,
    lastNoticeAt: 0,
  ...
  botState.groupRespondarState.set(groupId, {
    ...previous,
    mode: nextMode,
    winnerBotId,
    updatedAt: now,
  ...
  if (nextMode === "standby" && botId !== "main") {
    ...
    if (previous.winnerBotId === "main") {
      text =
        `⚠️ El bot principal no responda en este momento.\n` +
        `✅ *${botName}* se ativo como respaldo.\n` +
        `Cuando regrese el principal, este subbot volvera a silencio automático.`;
    } else {
      text =
  ...
  botState.groupRespondarState.set(groupId, {
    ...botState.groupRespondarState.get(groupId),
    lastNoticeAt: now,
  });
}
...
const GLOBAL_COMMAND_ALIAS_MAP = new Map([
  ...
  ["paineladmin", "gpainel"],
  ["adminpainel", "gpainel"],
  ["botpainel", "controlbot"],
  ["controlbot", "controlbot"],
  ["painelbot", "controlbot"],
  ["antierrovisible", "antierro"],
  ["erroes", "logs"],
  ["respaldo", "backup"],
  ["restaurar", "restore"],
  ["tablero", "dashboard"],
  ...
  ["rankdinheiro", "topdolares"],
  ["ranksolicitudes", "topsolicitudes"],
  ["enviardinheiro", "transferir"],
  ["saldo", "dolares"],
  ["cartera", "dolares"],
...
let OWNER_IDS = buildDonoIds(settings);
...
let totalMensagems = 0;
let totalComandos = 0;

const mensagemsPorTipo = {
  Grupo: 0,
  Privado: 0,
  Desconocido: 0,
};
...
function isTransientRuntimeErro(value) {
...
function recordFatalRuntimeErro(kind, payload) {
  if (isTransientRuntimeErro(payload)) {
    console.warn(
...
function runTaskWithTimeout(label, timeoutMs, task, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    Promise.resolve()
      ...
      .catch((erro) => {
        if (settled) {
          console.erro(`${label} termino con erro despues del timeout:`, erro);
          return;
        }
...
function abortActiveCommand(botState, reason = "cancelled") {
  ...
  try {
    if (!abortController.signal?.aborted) {
      abortController.abort(new Erro(String(reason || "cancelled")));
    }
  } catch {
    try {
      abortController.abort();
...
function wrapSocketSendMessage(botState, sock) {
  ...
  function isTransientSendErro(erro) {
...
async function sendCommandTimeoutNotice(context, erro) {
  ...
  try {
    ...
    console.erro("No pude avisar el timeout del comando:", notifyErro);
  }
}
...
function getSafeCommandErroText(erro) {
  ...
  if (
    ...
    return "Erro temporal del proveedor. Tente de novo en unos segundos.";
  }
  ...
  if (!raw) {
    return "Ocurrio un erro inaguardedo al ejecutar el comando.";
  }
...
async function sendVisibleCommandErroNotice(botState, context, erro) {
  if (!context?.sock || !context?.from) return;
  if (isTaskTimeoutErro(erro)) return;

  const visibility = getErroVisibilityState();
  ...
  if (!visibility.ownerDetails || context.esDono) {
    return;
  }
  ...
  const detailText =
    ...
    `Detalhe: ${String(erro?.message || erro || "sin detalhe").slice(0, 900)}`;
...
function getReconnectDelay(botState, options = false) {
  ...
  // Cortes temporales/aguardedos: recuperacion mas rapida para mejorar estabilidad percibida.
...
function logMessageUpsertEvent(botState, type, count) {
  ...
  logBotEvent(
    botState,
    "info",
    `Actividad mensagems: +${totalInWindow} eventos (${botState.lastUpsertType}) en ${seconds}s`
  );
}
...
function refreshBotConfigCache() {
  ...
  OWNER_IDS = buildDonoIds(settings);
  refreshChannelInfo();
...
function isMainPreLinkLoggedOut(botState, loggedOut) {
  ...
  // como posible falha temporal de reconexion (no como pre-vinculacion).
...
async function getMessageExecutionInfo(botState, sock, message) {
  const senderId = normalizeJidUser(message.sender);
  const esDono = OWNER_IDS.has(senderId);
  const info = {
    esDono,
    isDono: esDono,
    esAdmin: false,
...
async function runMessageHooks(botState, context) {
  for (const cmd of messageHookModules) {
    try {
      ...
      console.erro(`${getBotTag(botState)} Erro onMessage:`, err);
    }
  }
...
async function runGroupUpdateHooks(botState, sock, update) {
  for (const cmd of groupUpdateHookModules) {
    try {
      ...
      console.erro(`${getBotTag(botState)} Erro onGroupUpdate:`, err);
    }
  }
}
...
async function runMessageDeleteHooks(botState, sock, payload) {
  for (const cmd of messageDeleteHookModules) {
    try {
      ...
      console.erro(`${getBotTag(botState)} Erro onMessageDelete:`, err);
    }
  }
}
...
async function canRunCommand(cmd, context) {
  ...
  if (cmd?.ownerOnly && !context.esDono) {
    await context.sock.sendMessage(
      context.from,
      {
  ...
  if (disabledState.blocked && !context.esDono) {
    ...
    if (disabledState.lastErro) {
      text += `\nÚltimo erro: ${disabledState.lastErro}`;
    }
...
function getErroVisibilityState() {
  const mode = normalizeErroVisibilityMode(settings?.system?.erroVisibilityMode);
  return {
    mode,
    enabled: mode !== "off",
...
function setErroVisibilityMode(mode = "off") {
  ensureSystemSettings(settings);
  settings.system.erroVisibilityMode = normalizeErroVisibilityMode(mode);
  saveSettingsFile();
  refreshBotConfigCache();
  return getErroVisibilityState();
}
...
function abortActiveDownloadJobs(botState, reason = "cancelled") {
  ...
  const abortErro = new Erro(String(reason || "cancelled"));
  let abortedCount = 0;
...
async function isBlockedByMaintenance(cmd, context) {
  ...
  if (maintenance.ownerOnly) {
    ...
    text = "Las downloads estan en mantenimiento temporal. Tente otra vez mas tarde.";
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("data", (chunk) => {
      ...
      if (Buffer.byteLength(raw, "utf8") > maxBytes) {
        const erro = new Erro("El cuerpo excede el limite permitido.");
        erro.statusCode = 413;
        req.destroy(erro);
      }
    });
    ...
    req.on("end", () => {
      try {
        ...
        const erro = new Erro("JSON invalido.");
        erro.statusCode = 400;
        reject(erro);
      }
    });
...
function resolvePainelCallbackConfig(payload = {}) {
  const bodyCallbackUrl = String(payload?.painelPairingUrl || "").trim();
  const bodyCallbackToken = String(payload?.painelBotToken || "").trim();
...
function mapRuntimePairingResultToPainelPayload(requestToken, result = {}) {
  if (result?.ok) {
    return {
      requestToken,
...
async function sendPainelPairingUpdate(callbackUrl, callbackToken, payload) {
  ...
  if (!response.ok) {
    ...
    throw new Erro(
      `El painel respondio ${response.status} al callback del pairing. ${erroText}`.trim()
    );
  }
...
async function processInternalSubbotRequest(payload = {}) {
  ...
  let result = await global.botRuntime.requestBotPairingCode("subbot", {
    ...
    requesterJid: `painel:${requestToken}`,
  ...
  const { callbackUrl, callbackToken } = resolvePainelCallbackConfig(payload);
  const painelPayload = mapRuntimePairingResultToPainelPayload(requestToken, result);

  if (callbackUrl && callbackToken) {
    try {
      await sendPainelPairingUpdate(callbackUrl, callbackToken, painelPayload);
    } catch (erro) {
      console.erro("[internal-webhook] No pude enviar el callback al painel:", erro?.message || erro);
    }
  }
    ...
    setTimeout(async () => {
      ...
      if (!restartResult.ok && pm2Name !== "fsociety-bot") {
        ...
        console.erro(
          ...
          String(restartResult?.stderr || restartResult?.stdout || "sin detalhe").trim()
        );
        process.exit(0);
        return;
      }
  ...
  botState.reconnectTimer = setTimeout(() => {
    ...
    Promise.resolve(iniciarInstanciaBot(botState.config)).catch((erro) => {
      logBotEvent(
        botState,
        "erro",
        `Erro en reconexion programada: ${String(erro?.message || erro)}`
      );
...
function getHealthSnapshot() {
  ...
  return {
    ...
    counters: {
      totalMessages: Number(totalMensagems || 0),
...
function buildPrometheusMetrics() {
  ...
  lines.push(`fsociety_messages_total ${Number(totalMensagems || 0)}`);
...
function buildMainPairingSnapshot(result = null) {
  ...
  if (result?.status === "created" || result?.status === "cached") {
    ...
    pairingStatus = "processing";
  } else if (["erro", "missing_number", "missing_bot"].includes(String(result?.status || ""))) {
...
function readJsonRequestBody(req, maxBytes = 32768) {
  return new Promise((resolve, reject) => {
    ...
    req.on("data", (chunk) => {
      ...
      if (size > maxBytes) {
        reject(new Erro("payload_too_large"));
        req.destroy();
        return;
      }
...
function buildBridgeChatReply(mensagem = "", usuário = "web-user") {
  ...
  if (!normalized) {
    return "Escreva un mensagem para consultar el estado del bot.";
  }

  if (/(ajuda|help|menu|opções)/.test(normalized)) {
    return (
      `Olá ${usuário}. Desde esta web puedes consultar estado del bot, revisar capacidad y pedir un subbot.` +
      `\n\nComandos úteis:` +
      `\n- Escreva "estado general"` +
      `\n- Escreva "slots disponíveis"` +
      `\n- Use el formulario de solicitud de subbot con tu número` +
      `\n- En WhatsApp tambien puedes usar: ${prefix}subbot 51912345678`
    );
  }

  if (/(estado|status|resumo|health)/.test(normalized)) {
    return (
      `Estado del runtime:` +
      `\n- Bot principal: ${mainReady}` +
      `\n- Solicitudes publicas: ${subbotState.publicRequests ? "encendidas" : "apagadas"}` +
      `\n- Slots livres: ${Number(subbotState.availableSlots || 0)}` +
  ...
  if (/(subbot|slot|slots|capacidad|pairing|codigo)/.test(normalized)) {
    return (
      `Subbots disponíveis ahora:` +
      `\n- Public requests: ${subbotState.publicRequests ? "ON" : "OFF"}` +
      `\n- Slots livres: ${Number(subbotState.availableSlots || 0)}` +
      `\n- Slots ativos: ${Number(subbotState.activeSlots || 0)}` +
      `\n\nSi quieres un codigo real, use el formulario web o el comando ${prefix}subbot 51912345678.`
    );
  }

  return (
    `Recibi tu mensagem, ${usuário}. El bridge web esta conectado al runtime del bot.` +
    `\n\nSi quieres pedir un subbot, use el formulario de la web con tu número o el comando ${prefix}subbot 51912345678 en WhatsApp.`
  );
}

function resolveBridgeSubbotErro(result = {}, maxSlots = 15) {
  if (result?.status === "missing_bot") {
    return `No encontre ese slot. Use un valor entre 1 y ${maxSlots}.`;
  }

  if (result?.status === "no_capacity") {
    return "No hay slots livres para crear otro subbot ahora mismo.";
  }
...
async function handleBridgeRequest(req, res, requestUrl) {
  ...
  if (pathname === "/bridge/chat" && req.method === "POST") {
    ...
    sendDashboardJson(res, 200, {
      ...
      respuesta: buildBridgeChatReply(mensagem, usuário),
    });
    return true;
  }
  ...
  const número = normalizePairingPhoneNumber(
    ...
    sendDashboardJson(res, 200, {
      ...
      respuesta:
        `Solicitud aceptada. Use el codigo para vincular ${result.displayName || "tu subbot"} en WhatsApp.`,
      codigo: result.code,
    ...
    (async () => {
      ...
      if (pathname === "/internal/subbot/request") {
        ...
        try {
          ...
          const callbackConfig = resolvePainelCallbackConfig(body);
          ...
          if (hasAsyncCallback) {
            Promise.resolve()
              ...
              .catch((erro) => {
                console.erro(
                  "[internal-webhook] Erro processando solicitud:",
                  erro?.message || erro
                );
              });
            ...
            writeJson(res, 202, {
              ...
              pairingStatus: "processing",
              pairingMessage:
                "Solicitud aceptada por el bot principal. El codigo se enviara al painel cuando este listo.",
            });
            return;
          }
          ...
          writeJson(res, 200, {
            accepted: true,
            requestToken,
            ...outcome.painelPayload,
          });
          return;
        } catch (erro) {
          writeJson(res, erro?.statusCode || 500, {
            message:
              erro?.message || "No pude procesar la solicitud del subbot.",
          });
          return;
        }
      }
...
<body>
  ...
  <div class="card" style="margin-top:16px"><pre>${JSON.stringify(snapshot, null, 2)}</pre></div>
      ...
      if (erro?.message === "invalid_json") {
        sendDashboardJson(res, 400, {
          ok: false,
          erro: "JSON invalido en la solicitud del bridge.",
        });
        return;
      }
      ...
      if (erro?.message === "payload_too_large") {
        sendDashboardJson(res, 413, {
          ok: false,
          erro: "La solicitud al bridge es demais grande.",
        });
        return;
      }

      console.erro("Erro en dashboard/bridge:", erro);
...
function buildDashboardFrame(params = {}) {
  ...
  const compactProcess =
    rawProcess.includes("ÚNICO") || rawProcess.includes("HOSTING")
      ? "ÚNICO"
  ...
  lines.push(`╠${"═".repeat(contentWidth)}╣`);
  row(`  Dono   : ${ownerName.toUpperCase()}`);
...
function printPairingPromptPainel(botLabel = "MAIN") {
    ...
    for (const archivo of archivos) {
      ...
      try {
        ...
        if (LOG_COMMAND_LOADS) {
          console.log("Comando carregado:", nomes.join(", "));
        }
      } catch (err) {
        console.erro("Erro cargando comando:", ruta, err);
      }
    }
  }
...
function summarizeBotConfig(config) {
  ...
  if (persistedState) {
    return {
      ...
      lastSendErroAt: Number(persistedState.lastSendErroAt || 0),
      lastSendErro: String(persistedState.lastSendErro || ""),
      ...
      lastPairingErroAt: Number(persistedState.lastPairingErroAt || 0),
      lastPairingErro: String(persistedState.lastPairingErro || ""),
...
async function reconnectManagedSubbot(botId, options = {}) {
  ...
  if (!hasAnySessionOrAssignment) {
    return {
      ok: false,
      status: "slot_free",
      message: `El slot ${targetConfig.slot} esta livre y no necesita reconexion.`,
    };
  }
...
async function ensureSplitBotProcess(config = {}) {
  ...
  console.erro(`[PM2] No pude iniciar ${processName}:`, result.stderr || result.stdout);
  return false;
}
...
async function deleteSplitBotProcess(botId) {
  ...
  console.erro(`[PM2] No pude eliminar ${processName}:`, result.stderr || result.stdout);
  return false;
}
...
async function askPairingModeInConsole(options = {}) {
  ...
  if (!canPromptInConsole()) {
    // En PM2/hosting sin TTY interativo, permitimos forzar el modo por entorno.
  ...
  // Si NO hay sesion real, priorizamos menu interativo aunque exista PAIRING_MODE en entorno.
  ...
  console.log(chalk.bgBlack.redBright("╠════════════════════════════════════════════════════════════════════╣"));
  console.log(chalk.bgBlack.whiteBright("║  Consejo: si falla codigo, use QR por 30-40 min                  ║"));
  console.log(chalk.bgBlack.redBright("╚════════════════════════════════════════════════════════════════════╝"));

  let option = "";
  for (let i = 0; i < 3; i++) {
    option = String(await perguntarSeguro(chalk.greenBright("Escolha modo [1/2] > ")))
      .trim()
      .toLowerCase();
    if (option === "1" || option === "2") {
      break;
    }
    console.log(chalk.redBright("Opção inválida. Escreva 1 o 2."));
  }
  ...
  console.log(
    chalk.cyanBright(
      runtimePairingMode === "code"
        ? "Modo selecionedo: NUMERO + CODIGO"
        : "Modo selecionedo: QR"
    )
  );
  ...
  while (!resolvedNumber) {
    ...
    console.log(chalk.redBright("Número invalido. Use 10 a 15 digitos con codigo de pais."));
  }

  if (!resolvedNumber) {
    console.log(
      chalk.yellowBright(
        "No se guardo número. Se solicitara luego cuando pidas codigo nuevamente."
      )
    );
    return;
  }
...
function shouldDeferManagedStop(botState, stopReason = "aguardendo_sesion") {
  if (!botState || stopReason !== "aguardendo_sesion") {
    return false;
  }
...
function syncSettingsFromDisk() {
  try {
    ...
    console.erro("Erro recargando settings:", err);
    return false;
  }
}
...
async function syncManagedProcessBots() {
  ...
  for (const config of getManagedProcessBotConfigs()) {
    try {
      ...
      if (!startDecision.start) {
        const stopReason = config.enabled === false ? "slot_apagado" : "aguardendo_sesion";
      ...
      console.erro(
        `[SYNC ${config?.id || "bot"}] Erro sincronizando bot:`,
        erro?.message || erro
      );
    }
  }
...
async function requestPairingCode(botState, options = {}) {
  ...
  if (botState.config?.id !== "main" && !isMainBotReady()) {
    return {
      ...
      message: "Primeiro vincula y conecta el bot principal desde la consola.",
    };
  }
  ...
  if (!resolvedNumber && allowPrompt) {
    if (!botState.pairingPromptShown) {
      printMaskPairingScreen();
      printPairingPromptPainel(botState?.config?.label || "MAIN");
  ...
  if (!resolvedNumber) {
    ...
    return {
      ...
      message:
        `Debes enviar un número valido con codigo de pais (10 a 15 digitos). ` +
  ...
  if (isPairingCooldownActive(botState)) {
    ...
    return {
      ...
      status: "cooldown_405",
      message:
        `WhatsApp aplico una pause temporal para este número. ` +
        `Aguarde aprox ${waitMin} min antes de pedir otro codigo.`,
    };
  }

  // Persistimos el número tan pronto sea valido para evitar pedirlo en bucle
  ...
  if (!sock) {
    return {
      ok: false,
      status: "unavailable",
      message: `${botState.config.displayName} aun se esta iniciando. Tente de novo en unos segundos.`,
    };
  }
  ...
  try {
    ...
    if (!socketReady) {
      ...
      return {
        ...
        message:
          "La conexion con WhatsApp aun no esta lista. Aguarde unos segundos y vuelve a intentar una sola vez.",
      };
    }
    await delay(1200);
...
async function waitForRemoteBotPairing(targetConfig, timeoutMs = REMOTE_PAIRING_WAIT_MS) {
  ...
  return {
    ...
    message:
      ...
      `Tente otra vez en unos segundos.`,
  };
}
  ...
  if (result.ok) {
    ...
    console.log(
      chalk.bgBlack.cyanBright(
        `║  NUMERO : +${String(result.number || "sin_número").padEnd(50, " ")}║`
      )
    );
    console.log(chalk.bgBlack.redBright("╠════════════════════════════════════════════════════════════════════╣"));
    console.log(
    ...
    console.log(
      chalk.bgBlack.white(
        "║  Si sale invalido, aguarde 30-40 min y retente solo una vez      ║"
      )
    );
    console.log(chalk.bgBlack.redBright("╚════════════════════════════════════════════════════════════════════╝"));
    return;
  ...
  if (result.status === "socket_not_ready") {
    ...
    if (silentConsoleMode) {
      ...
      if (shouldShowPairingNotice(botState, 25000)) {
        console.log(
          `${getBotTag(botState)} Aun conectando con WhatsApp... seguire retentando hasta mostrar el codigo.`
        );
      }
      return;
    }
...
async function acceptGroupInviteWithRetry(sock, inviteCode, options = {}) {
  ...
  throw lastErro || new Erro("No pude aceptar la invitacion del grupo.");
}
...
function buildManagedGroupJoinNotice(botState, isAdmin = false) {
  ...
  if (botId === "main") {
    if (isAdmin) {
      return (
        ...
        `Ya tengo administrador aqui, asi que respondare normalmente.\n` +
        `Si el bot presenta alguna falla, pueden avisar por este grupo de suporte.\n` +
        `Si quieres silenciarme aqui use *${prefix}botgrupo off*.`
      );
    }
    ...
    return (
      `🤖 *${botName}* ya se unio al grupo principal de suporte.\n\n` +
      `Si el bot presenta alguna falla, pueden avisar por este grupo de suporte.\n` +
      `En este momento no respondare mensagems porque no tengo administrador.\n` +
      `Estado atual: *BOT OFF*.`
    );
  }
  ...
  if (isAdmin) {
    return (
      ...
      `Tengo administrador aqui, asi que respondare mensagems normalmente.\n` +
      `Si quieres silenciarme en este grupo use *${prefix}botgrupo off*.`
    );
  }

  return (
    `🤖 *${botName}* ya entro a este grupo.\n\n` +
    `En este grupo no voy a respondar mensagems porque no tengo administrador.\n` +
    `Estado atual: *BOT OFF*.\n` +
    `Pide al owner que me de admin para ativarme automaticamente.`
  );
}
...
async function applyManagedGroupPolicy(botState, sock, groupId, options = {}) {
  ...
  const sent = await sendGroupRespondarNotice(
    sock,
    groupId,
...
async function ensureBotAutoJoinGroup(botState) {
  ...
  botState.autoJoinInFlight = (async () => {
    ...
    logBotEvent(
      botState,
      "info",
      `Autojoin iniciado: tentando entrar al ${targetLabel}.`
    );
    ...
    try {
      ...
      if (!alreadyJoined) {
        logBotEvent(
          botState,
          "warn",
          `Autojoin de grupo no concluído: ${String(erro?.message || erro).slice(0, 180)}`
        );
        return null;
      }
    ...
    logBotEvent(
      ...
      "success",
      joinedNow
        ? `Autojoin concluído en ${targetLabel}: ${targetGroupId}`
...
async function joinGroupInviteAllSubbots(inviteCode, options = {}) {
  ...
  if (!code) {
    return {
      ...
      message: "Falta el link/codigo de invitacion.",
      results: [],
    };
  }
...
global.botRuntime = {
      ...
      return {
        ...
        message:
          requestedBotId === "subbot"
            ? "No hay slots livres para crear otro subbot ahora mismo."
    ...
    if (targetConfig.id !== "main") {
      ...
      if (!nextPairingNumber && !assignedNumber && !slotBusy) {
        return {
          ok: false,
          status: "missing_number",
          message: "No pude detectar el número para este subbot.",
        };
      }
  ...
  getErroVisibilityState: () => getErroVisibilityState(),
  setErroVisibilityMode: (mode = "off") => setErroVisibilityMode(mode),
...
async function handleIncomingMessages(botState, sock, messages) {
  for (const raw of messages || []) {
    ...
    try {
      ...
      mensagemsPorTipo[tipo] = (mensagemsPorTipo[tipo] || 0) + 1;
      ...
      await maybeAnnounceRespondarTransition(botState, sock, raw, groupReservation);
      ...
      if (isDownloadCommand(cmd)) {
        ...
        runningJob.promise.catch(async (err) => {
          ...
          await sendVisibleCommandErroNotice(botState, commandContext, err);
          console.erro(`${getBotTag(botState)} Erro comando concurrente [${activeRequestId || "n/a"}]:`, err);
        });
        continue;
      }
      ...
      if (activeCommandContext) {
        await sendVisibleCommandErroNotice(botState, activeCommandContext, err);
      }
      console.erro(`${getBotTag(botState)} Erro comando [${activeRequestId || "n/a"}]:`, err);
    }
  }
}
...
async function iniciarInstanciaBot(config) {
  ...
  if (isPreLink405Paused(botState) && shouldHardStopOnPreLink405(botState)) {
    ...
    logBotEvent(
      botState,
      "warn",
      `Inicio bloqueado por 405 temporal. Aguarde aprox ${Math.ceil(waitMs / 60000)} min.`
    );
    return;
  }
  ...
  try {
      ...
      try {
        ...
        if (
          ...
          const qrHint = qrFallback
            ? "Modo QR ativo por bloqueo 405. Escanea el QR para vincular y evitar el limite por número."
        ...
        if (connection === "open") {
          ...
          ensureBotAutoJoinGroup(botState).catch((erro) => {
            logBotEvent(
              botState,
              "warn",
              `No pude aplicar autoingreso de grupo: ${String(erro?.message || erro).slice(0, 180)}`
            );
          });

          if (botState.config?.id === "main") {
            startSecondaryBots().catch((erro) => {
              console.erro("[SECONDARY] Erro iniciando subbots:", erro?.message || erro);
            });
          }
        }
        ...
        if (connection === "close") {
            ...
            if (!silencePreLinkLogs) {
              ...
              if (now - Number(botState.lastManualRestartCloseLogAt || 0) >= 8_000) {
                ...
                logBotEvent(
                  botState,
                  "info",
                  "Cierre controlado por .restart detectado. Espero reconexion programada sin duplicar tentativas."
                );
              }
            }
            return;
          ...
          if (connectionReplaced) {
            ...
            logBotEvent(
              botState,
              "warn",
              `Sesion reemplazada (440). No reconecto en bucle; revisa si ese número ` +
            ...
            if (!silencePreLinkLogs) {
              if (shouldHardStopOnPreLink405(botState)) {
                ...
                logBotEvent(
                  botState,
                  "warn",
                  "Activando modo QR temporal. Use escaneo QR en este periodo para vincular."
                );
              }
            }
            ...
            if (shouldHardStopOnPreLink405(botState)) {
              ...
              if (!silencePreLinkLogs) {
                logBotEvent(
                  botState,
                  "warn",
                  "Pause de segurança activada: no reconectare en automático tras 405 pre-vinculacion."
                );
                logBotEvent(
                  botState,
                  "warn",
                  "Aguarde 40 min, cambia IP/red (si es posible) y reinicia manualmente el bot."
                );
              }
              return;
            }
          ...
          if (isMainPreLinkLoggedOut(botState, loggedOut)) {
            ...
            logBotEvent(
              botState,
              "warn",
              "No borre auth automaticamente. Si la sesion sigue vigente, use `.restart` y aguarde 30-60s."
            );
            return;
          }
          ...
          if (loggedOut && botState.config?.id === "main") {
            ...
            if (!shouldAutoResetAuthOnPersistent401(botState)) {
              ...
              logBotEvent(
                botState,
                "warn",
                "Use `.restart` para reintento suave. Solo re-vincula si WhatsApp realmente cerro la sesion."
              );
              return;
            }
            ...
            removeAuthFolder(config.authFolder);
            // Si la sesion guardada ya no sirve, reabrimos el selector interativo
            // para que el usuário elija QR o número+codigo antes del próximo intento.
            runtimePairingMode = "";
          ...
          if (pairingRejected405 && !silencePreLinkLogs) {
            logBotEvent(
              botState,
              "warn",
              "WhatsApp devolvio 405. Pauso auto-pairing por 40 min para evitar bloqueo por retentativas."
            );
          }
    ...
    boundSock.ev.on("messages.delete", async (update) => {
      ...
      for (const key of keys) {
        try {
          ...
          console.erro(`${getBotTag(botState)} Erro processando message delete:`, err);
        }
      }
    });
  } catch (err) {
    ...
    logBotEvent(config, "erro", `Erro iniciando bot: ${String(err?.message || err)}`);
    writePersistedBotRuntimeState(botState);
  ...
  if (!managedBotSyncInterval) {
    managedBotSyncInterval = setInterval(() => {
      (async () => {
        ...
        console.erro("Erro sincronizando procesos del bot:", err);
      });
  ...
  if (!autoCleanInterval) {
    autoCleanInterval = setInterval(() => {
      try {
        ...
        console.erro("Erro en autoclean:", err);
      }
    }, 60_000);
  ...
  if (!botHealthCheckInterval) {
    botHealthCheckInterval = setInterval(() => {
      try {
        ...
        console.erro("Erro en bot healthcheck:", err);
      }
