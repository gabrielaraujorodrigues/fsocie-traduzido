...
export function assertSubbotCommandAllowed(ctx = {}, commandName = "") {
  ...
  throw new Erro(
    `Eres subbot y este comando pesado solo esta habilitado en el bot principal. ` +
      ...
      `Si necesitas mas capacidad, pide al owner que aumente el limite o activa VIP para downloads sin limite.`
  );
}
...
export function assertDownloadWithinPolicy(ctx = {}, bytes = 0, label = "archivo") {
  ...
  throw new Erro(
    `Eres subbot y el limite de download es ${humanBytes(policy.maxBytes)}. ` +
      ...
      `Pide al owner que aumente el limite o use VIP para downloads sin limite.`
  );
}
